"""
Image canvas with zoom, pan, and annotation capabilities
"""

import numpy as np
from PyQt6.QtWidgets import (QGraphicsView, QGraphicsScene, QGraphicsPixmapItem,
                             QGraphicsTextItem, QMenu, QGraphicsRectItem,
                             QGraphicsEllipseItem, QGraphicsItem, QGraphicsLineItem,
                             QWidget, QHBoxLayout, QPushButton)
from PyQt6.QtCore import Qt, pyqtSignal, QPointF, QRectF, QTimer
from PyQt6.QtGui import (QPixmap, QImage, QPen, QBrush, QColor, QPainter,
                        QTransform, QCursor, QPolygonF, QFont, QAction)
from pathlib import Path
import cv2
import time


class OutwardBBoxRectItem(QGraphicsRectItem):
    """BBox rectangle whose painted stroke stays outside the stored rectangle."""

    def boundingRect(self):
        pen = self.pen()
        margin = pen.widthF() if pen.style() != Qt.PenStyle.NoPen else 0
        return self.rect().adjusted(-margin, -margin, margin, margin)

    def paint(self, painter, option, widget=None):
        pen = self.pen()
        painter.setPen(pen)
        painter.setBrush(self.brush())

        width = pen.widthF()
        if width <= 0 or pen.style() == Qt.PenStyle.NoPen:
            painter.drawRect(self.rect())
            return

        # QGraphicsRectItem normally centers the stroke on rect(). Expanding the
        # painted rect by half the pen width makes the stroke's inner edge match
        # rect(), so the true bbox pixels are not covered by the line.
        visual_rect = self.rect().adjusted(-width / 2, -width / 2,
                                           width / 2, width / 2)
        painter.drawRect(visual_rect)


class ImageCanvas(QGraphicsView):
    """Image canvas with zoom, pan, and annotation capabilities"""
    
    point_clicked = pyqtSignal(int, int, bool)  # x, y, is_positive
    box_drawn = pyqtSignal(int, int, int, int)  # x1, y1, x2, y2
    annotation_changed = pyqtSignal()
    masks_visibility_changed = pyqtSignal(bool)  # Signal when mask visibility changes
    bboxes_visibility_changed = pyqtSignal(bool)  # Signal when bbox visibility changes
    instance_visibility_changed = pyqtSignal(int, str, bool)  # instance_id, category, visible
    instance_visibility_toggle_blocked = pyqtSignal(int, str)  # instance_id, category
    instance_selected = pyqtSignal(int, str)  # instance_id, category
    brush_size_step_requested = pyqtSignal(int)  # +1 larger, -1 smaller
    brush_size_unit_step_requested = pyqtSignal(int)  # exact +1 larger, -1 smaller
    brush_eraser_toggle_requested = pyqtSignal()
    instance_switch_tap_progress = pyqtSignal(int, str, int)  # instance_id, category, taps remaining
    new_instance_requested = pyqtSignal(str)  # category
    
    def __init__(self, parent=None):
        super().__init__(parent)
        
        self.scene = QGraphicsScene(self)
        self.setScene(self.scene)
        
        # Setup view
        self.setTransformationAnchor(QGraphicsView.ViewportAnchor.AnchorUnderMouse)
        self.setResizeAnchor(QGraphicsView.ViewportAnchor.AnchorUnderMouse)
        self.setDragMode(QGraphicsView.DragMode.ScrollHandDrag)
        self.setRenderHint(QPainter.RenderHint.Antialiasing)
        self.setRenderHint(QPainter.RenderHint.SmoothPixmapTransform)
        self.setMouseTracking(True)
        self.viewport().setMouseTracking(True)
        
        # Image
        self.image_item = None
        self.current_image = None
        self.image_path = None
        
        # Tool state
        self.current_tool = 'pan'
        self.brush_size = 10
        self.mask_opacity = 64  # Default opacity for masks (0-255) - 25%
        
        # Annotation type - determines which category is being added
        self.current_annotation_type = 'bee'  # 'bee', 'chamber', 'hive', or 'pollen'
        
        # Annotations - Separate masks allow overlapping between bee/chamber/hive/pollen
        self.bee_mask = None  # H×W int32 array with bee instance IDs
        self.chamber_mask = None  # H×W int32 array with chamber instance IDs
        self.hive_mask = None  # H×W int32 array with hive instance IDs
        self.pollen_mask = None  # H×W int32 array with pollen instance IDs
        
        # Legacy compatibility: provide combined view of all masks (readonly, dynamically generated)
        self._combined_mask_cache = None
        self._combined_mask_dirty = True
        
        self.mask_items = []  # List of QGraphicsPixmapItem for visualization
        self.mask_colors = {}  # Dict mapping instance_id -> (r,g,b) color
        self.next_mask_id = 1  # Counter for assigning unique IDs
        self.selected_mask_idx = -1
        self.selected_instance_category = None
        self.masks_visible = True  # Track mask visibility state
        self.label_items = []  # List of QGraphicsTextItem for instance labels
        self.labels_visible = True  # Track label visibility state
        self.labels_visible_before_hiding = True  # Remember label state while masks are toggled off
        self.aruco_label_items = []  # List of QGraphicsTextItem for ArUco labels
        self.aruco_labels_visible = False
        self.aruco_instance_labels = {}  # Dict mapping instance_id -> ArUco ID
        self.annotation_metadata = {}  # Dict mapping instance_id -> metadata dict (for marker data, etc.)
        
        # Annotation type visibility and colors
        self.annotation_type_colors = {
            'bee': None,  # Random colors for bee instances
            'chamber': (255, 0, 0),  # Red for chamber
            'hive': (255, 255, 0),  # Yellow for hive
            'pollen': (255, 165, 0)  # Orange for pollen
        }
        # Initialize visibility to match toolbar checkbox defaults (bees=True, others=False)
        self.annotation_type_visibility = {
            'bee': True,
            'chamber': False,
            'hive': False,
            'pollen': False
        }
        
        # Performance optimization: cache instance IDs and bounding boxes
        self._cached_instance_ids = None  # Cached sorted list of instance IDs
        self._cached_bboxes = {}  # Dict mapping instance_id -> (x, y, w, h)
        
        # Performance optimization: cache instance IDs and bounding boxes
        self._cached_instance_ids = None  # Cached sorted list of instance IDs
        self._cached_bboxes = {}  # Dict mapping instance_id -> (x, y, w, h)
        
        # Performance optimization: cache instance IDs and bounding boxes
        self._cached_instance_ids = None  # Cached sorted list of instance IDs
        self._cached_bboxes = {}  # Dict mapping instance_id -> (x, y, w, h)
        
        # Editing isolation - temporary mask for current edit
        self.editing_mask = None  # Binary H×W mask for instance being edited
        self.editing_instance_id = -1  # ID of instance being edited
        self.editing_instance_category = None
        self._editing_started_with_pixels = False
        self.editing_mask_item = None  # Visualization item for editing mask
        self.editing_bbox_item = None  # Visualization item for active editing bbox
        self._cached_overlay = None  # Cached RGBA overlay for regular masks
        self._editing_overlay_cache = None  # Cached RGBA overlay for editing mask
        self._is_actively_drawing = False  # Flag to skip expensive updates during drawing
        self.brush_history_limit = 10
        self.brush_undo_stack = []
        self.brush_redo_stack = []
        self._brush_step_active = False
        self._brush_step_start_snapshot = None
        self.brush_bbox_guard_extra_padding = 80
        self.brush_bbox_guard_max_padding = 240
        self.brush_cursor_preview_enabled = True
        self.brush_cursor_items = []
        self._last_brush_cursor_pos = None
        self._switch_tap_required = 3
        self._switch_tap_max_interval_seconds = 0.8
        self._switch_tap_candidate = None
        self._switch_tap_count = 0
        self._switch_tap_last_time = 0.0
        self._pending_switch_tap = None
        self._tap_move_tolerance_px = 8.0
        
        # Throttling for brush/eraser to improve performance
        self._pending_viz_update = False  # Flag indicating viz update is pending
        self._dirty_pixels = None  # Accumulate dirty pixels for batched update
        self._viz_update_timer = QTimer()
        self._viz_update_timer.setSingleShot(True)
        self._viz_update_timer.setInterval(16)  # ~60fps max update rate
        self._viz_update_timer.timeout.connect(self._flush_pending_viz_update)
        
        # SAM2 prompts
        self.positive_points = []  # List of (x, y) tuples
        self.negative_points = []  # List of (x, y) tuples
        self.prompt_items = []  # List of graphics items for visualization
        self.active_sam2_mask_idx = -1  # Index of mask being edited with SAM2 prompts
        
        # Selection marker. The variable name is kept for compatibility with
        # the existing selection cleanup/show-hide paths.
        self.selection_border_item = None
        
        # Drawing state
        self.is_drawing = False
        self.drawing_start = None
        self.temp_item = None
        
        # Inference box (for box inference mode)
        self.inference_box_rect = None  # QRectF or None
        self.inference_box_item = None  # QGraphicsRectItem
        self.inference_box_handles = []  # Corner/edge handle items
        self.dragging_box = False
        self.dragging_handle = None  # 'tl', 'tr', 'bl', 'br', 'move'
        self.drag_offset = None
        
        # View toggles
        self.show_segmentations = True  # Whether to show segmentation masks
        self.show_bboxes = False  # Whether to show bounding boxes
        self.hidden_instance_keys = set()  # Per-frame individual object display toggles
        self.hidden_bbox_instance_ids = set()  # Legacy alias for older bbox-only paths
        self._space_restore_visibility = None  # Previous layer state for Space overlay toggle
        self._view_restore_token = 0  # Guards stale queued fit/restore callbacks
        
        # Bbox editing (for bbox annotation mode)
        self.bbox_items_map = {}  # Dict mapping mask_id -> QGraphicsRectItem
        self.drawing_new_bbox = False  # Whether currently drawing a new bbox
        self.selected_bbox_id = None  # ID of currently selected bbox
        self.bbox_handles = []  # Corner handle items for selected bbox
        self.dragging_bbox = False
        self.dragging_bbox_handle = None  # 'tl', 'tr', 'bl', 'br', 'move'
        self.bbox_drag_offset = None
        self.bbox_original_rect = None  # Original rect before editing
        
        # Undo/redo
        self.history = []
        self.history_idx = -1
        
        self.zoom_factor = 1.0
        
        # Enable focus to receive keyboard events
        self.setFocusPolicy(Qt.FocusPolicy.StrongFocus)
        self._create_brush_size_overlay()
        
    def keyPressEvent(self, event):
        """Handle key press events"""
        if event.key() == Qt.Key.Key_Space:
            if event.isAutoRepeat():
                event.accept()
                return

            if event.modifiers() & Qt.KeyboardModifier.ShiftModifier:
                self.toggle_selected_instance_visibility()
            else:
                self.toggle_annotation_overlays()
            event.accept()
        elif (event.modifiers() & Qt.KeyboardModifier.ControlModifier and
              event.key() in (Qt.Key.Key_Plus, Qt.Key.Key_Equal) and
              not event.isAutoRepeat()):
            self.zoom_in()
            event.accept()
        elif (event.modifiers() & Qt.KeyboardModifier.ControlModifier and
              event.key() in (Qt.Key.Key_Minus, Qt.Key.Key_Underscore) and
              not event.isAutoRepeat()):
            self.zoom_out()
            event.accept()
        elif event.key() in (Qt.Key.Key_Plus, Qt.Key.Key_Equal) and not event.isAutoRepeat():
            self.brush_size_step_requested.emit(1)
            event.accept()
        elif event.key() in (Qt.Key.Key_Minus, Qt.Key.Key_Underscore) and not event.isAutoRepeat():
            self.brush_size_step_requested.emit(-1)
            event.accept()
        elif event.key() == Qt.Key.Key_F and not event.isAutoRepeat():
            # Fit image to window (reset zoom)
            self.fit_to_window()
        else:
            super().keyPressEvent(event)
    
    def keyReleaseEvent(self, event):
        """Handle key release events"""
        if event.key() == Qt.Key.Key_Space:
            event.accept()
        else:
            super().keyReleaseEvent(event)

    def _create_brush_size_overlay(self):
        """Create floating +/- brush-size controls over the viewport."""
        self._brush_size_overlay = QWidget(self.viewport())
        self._brush_size_overlay.setObjectName("brushSizeOverlay")
        self._brush_size_overlay.setAttribute(Qt.WidgetAttribute.WA_StyledBackground, True)
        self._brush_size_overlay.setFocusPolicy(Qt.FocusPolicy.NoFocus)

        layout = QHBoxLayout(self._brush_size_overlay)
        layout.setContentsMargins(6, 6, 6, 6)
        layout.setSpacing(6)

        self._brush_decrease_button = self._create_brush_size_button("-", -1)
        self._brush_increase_button = self._create_brush_size_button("+", 1)
        layout.addWidget(self._brush_decrease_button)
        layout.addWidget(self._brush_increase_button)

        self._brush_size_overlay.setStyleSheet("""
            QWidget#brushSizeOverlay {
                background-color: rgba(30, 34, 42, 190);
                border: 1px solid rgba(255, 255, 255, 80);
                border-radius: 8px;
            }
            QPushButton {
                background-color: rgba(255, 255, 255, 235);
                border: 1px solid rgba(20, 24, 30, 120);
                border-radius: 5px;
                color: rgb(20, 24, 30);
                font-size: 22px;
                font-weight: 700;
                min-width: 36px;
                max-width: 36px;
                min-height: 34px;
                max-height: 34px;
            }
            QPushButton:hover {
                background-color: rgb(255, 246, 170);
            }
            QPushButton:pressed {
                background-color: rgb(238, 221, 94);
            }
        """)
        self._brush_size_overlay.adjustSize()
        self._position_brush_size_overlay()
        self._brush_size_overlay.show()

    def _create_brush_size_button(self, label, direction):
        button = QPushButton(label, self._brush_size_overlay)
        button.setFocusPolicy(Qt.FocusPolicy.NoFocus)
        button.setToolTip("Decrease brush size by 1" if direction < 0 else "Increase brush size by 1")
        button.clicked.connect(lambda checked=False, step=direction: self._request_brush_size_step(step))
        return button

    def _request_brush_size_step(self, direction):
        self.brush_size_unit_step_requested.emit(direction)
        self.setFocus()

    def _position_brush_size_overlay(self):
        if not hasattr(self, '_brush_size_overlay') or self._brush_size_overlay is None:
            return
        self._brush_size_overlay.adjustSize()
        viewport_rect = self.viewport().rect()
        x = int((viewport_rect.width() - self._brush_size_overlay.width()) / 2 + 110)
        y = int(viewport_rect.height() - self._brush_size_overlay.height() - 24)
        x = max(8, min(x, viewport_rect.width() - self._brush_size_overlay.width() - 8))
        y = max(8, y)
        self._brush_size_overlay.move(x, y)
        self._brush_size_overlay.raise_()

    def _clear_brush_cursor_preview(self):
        """Remove the brush footprint cursor from the scene."""
        for item in getattr(self, 'brush_cursor_items', []):
            try:
                if item.scene() == self.scene:
                    self.scene.removeItem(item)
            except (RuntimeError, AttributeError):
                pass
        self.brush_cursor_items = []

    def _brush_cursor_preview_should_show(self):
        return (
            self.brush_cursor_preview_enabled and
            self.current_tool in {'brush', 'eraser'} and
            self.current_image is not None
        )

    def _set_default_cursor_for_current_tool(self):
        """Set the native cursor for the active tool."""
        if self.current_tool == 'pan':
            self.setCursor(Qt.CursorShape.OpenHandCursor)
        elif self.current_tool in {
            'sam2_prompt',
            'brush',
            'eraser',
            'sam2_box',
            'inference_box',
            'bbox'
        }:
            self.setCursor(Qt.CursorShape.CrossCursor)
        else:
            self.setCursor(Qt.CursorShape.ArrowCursor)

    def _ensure_brush_cursor_preview_items(self):
        """Create scene items used for the brush footprint cursor."""
        if len(self.brush_cursor_items) == 3:
            return

        self._clear_brush_cursor_preview()

        circle = QGraphicsEllipseItem()
        h_line = QGraphicsLineItem()
        v_line = QGraphicsLineItem()

        circle.setZValue(251)
        h_line.setZValue(250)
        v_line.setZValue(250)

        for item in (circle, h_line, v_line):
            item.setAcceptedMouseButtons(Qt.MouseButton.NoButton)
            self.scene.addItem(item)

        self.brush_cursor_items = [circle, h_line, v_line]

    def _update_brush_cursor_preview(self, scene_pos=None):
        """Show a brush-size cursor matching the actual image-space stroke."""
        if scene_pos is not None:
            self._last_brush_cursor_pos = QPointF(scene_pos)

        if not self._brush_cursor_preview_should_show() or self._last_brush_cursor_pos is None:
            self._clear_brush_cursor_preview()
            self._set_default_cursor_for_current_tool()
            return

        x = float(self._last_brush_cursor_pos.x())
        y = float(self._last_brush_cursor_pos.y())
        h, w = self.current_image.shape[:2]
        if not (0 <= x < w and 0 <= y < h):
            self._clear_brush_cursor_preview()
            self._set_default_cursor_for_current_tool()
            return

        self.setCursor(Qt.CursorShape.BlankCursor)
        self._ensure_brush_cursor_preview_items()
        circle, h_line, v_line = self.brush_cursor_items

        radius = max(0.5, float(self.brush_size) / 2.0)
        diameter = radius * 2.0
        circle.setRect(x - radius, y - radius, diameter, diameter)
        h_line.setLine(x - radius, y, x + radius, y)
        v_line.setLine(x, y - radius, x, y + radius)

        if self.current_tool == 'eraser':
            color = QColor(255, 80, 80, 235)
        else:
            color = QColor(255, 246, 80, 235)

        pen = QPen(color)
        pen.setWidthF(1.5)
        pen.setCosmetic(True)
        circle.setPen(pen)
        circle.setBrush(QBrush(Qt.BrushStyle.NoBrush))

        line_pen = QPen(color)
        line_pen.setWidthF(1.0)
        line_pen.setCosmetic(True)
        h_line.setPen(line_pen)
        v_line.setPen(line_pen)

    def set_brush_cursor_preview_enabled(self, enabled):
        """Enable or disable the brush-size cursor preview."""
        self.brush_cursor_preview_enabled = bool(enabled)
        self._update_brush_cursor_preview()
        if not self.brush_cursor_preview_enabled:
            self._set_default_cursor_for_current_tool()
    
    def hide_masks(self):
        """Hide all masks until toggled visible again."""
        self.set_show_segmentations(False)
    
    def show_masks(self):
        """Show all masks again"""
        self.set_show_segmentations(True)

    def toggle_annotation_overlays(self):
        """Toggle segmentation and bbox layers without changing class/instance switches."""
        if self.show_segmentations or self.show_bboxes:
            self._space_restore_visibility = (self.show_segmentations, self.show_bboxes)
            self.set_annotation_overlay_visibility(False, False)
            return False

        show_segmentations, show_bboxes = self._space_restore_visibility or (True, True)
        self._space_restore_visibility = None
        self.set_annotation_overlay_visibility(show_segmentations, show_bboxes)
        return bool(show_segmentations or show_bboxes)

    def _segmentation_overlays_visible(self):
        """Return whether segmentation-derived visuals should be shown."""
        return self.show_segmentations and self.masks_visible

    def _normalize_color(self, color):
        """Return a plain RGB tuple or None."""
        if color is None:
            return None
        if isinstance(color, np.ndarray):
            return tuple(int(c) for c in color.tolist())
        return tuple(int(c) for c in color)

    def color_for_category(self, category, instance_id=None, preferred_color=None):
        """Return the display color for a category/instance.

        Bees keep per-instance colors. Chamber, hive, and pollen always use
        fixed category colors so their labels cannot drift after category edits.
        """
        fixed_color = self.annotation_type_colors.get(category)
        if fixed_color is not None:
            return fixed_color

        if preferred_color is not None:
            return self._normalize_color(preferred_color)

        if instance_id is not None and instance_id in self.mask_colors:
            return self._normalize_color(self.mask_colors[instance_id])

        return tuple(np.random.randint(0, 255, 3).tolist())

    def ensure_instance_color(self, instance_id, category=None, preferred_color=None):
        """Store and return the correct display color for an instance."""
        if category is None:
            category = self.annotation_metadata.get(instance_id, {}).get('category', 'bee')
        color = self.color_for_category(
            category,
            instance_id=instance_id,
            preferred_color=preferred_color
        )
        self.mask_colors[instance_id] = color
        return color

    def _event_view_pos(self, event):
        if hasattr(event, 'position'):
            return QPointF(event.position())
        return QPointF(event.pos())

    def _reset_instance_switch_tap_sequence(self):
        self._switch_tap_candidate = None
        self._switch_tap_count = 0
        self._switch_tap_last_time = 0.0

    def _cancel_pending_instance_switch_tap(self, reset_sequence=False):
        self._pending_switch_tap = None
        if reset_sequence:
            self._reset_instance_switch_tap_sequence()

    def _active_brush_instance(self):
        if self.editing_instance_id > 0:
            return self.editing_instance_id, (
                self.editing_instance_category or
                self.annotation_metadata.get(self.editing_instance_id, {}).get('category', self.current_annotation_type)
            )
        if self.selected_mask_idx > 0:
            return self.selected_mask_idx, (
                self.selected_instance_category or
                self.annotation_metadata.get(self.selected_mask_idx, {}).get('category', self.current_annotation_type)
            )
        return -1, None

    def _is_active_brush_instance(self, instance_id, category):
        active_id, active_category = self._active_brush_instance()
        if active_id != instance_id:
            return False
        return category is None or active_category is None or category == active_category

    def _active_brush_bbox_rect(self):
        active_id, _ = self._active_brush_instance()
        if active_id <= 0:
            return None

        if active_id in self.bbox_items_map:
            try:
                return QRectF(self.bbox_items_map[active_id].rect())
            except (RuntimeError, AttributeError):
                pass

        bbox = self.annotation_metadata.get(active_id, {}).get('bbox')
        if not bbox or len(bbox) < 4:
            return None

        x, y, width, height = bbox[:4]
        width = float(width)
        height = float(height)
        if width <= 0 or height <= 0:
            return None

        return QRectF(float(x), float(y), width, height)

    def _can_start_brush_stroke_at(self, scene_pos):
        bbox_rect = self._active_brush_bbox_rect()
        if bbox_rect is None:
            return True

        brush_radius = max(1.0, float(self.brush_size) / 2.0)
        padding = min(
            float(self.brush_bbox_guard_max_padding),
            brush_radius + float(self.brush_bbox_guard_extra_padding)
        )
        padded_rect = bbox_rect.adjusted(-padding, -padding, padding, padding)
        return padded_rect.contains(scene_pos)

    def _brush_switch_target_at(self, scene_pos):
        x, y = int(scene_pos.x()), int(scene_pos.y())

        if (
            self.editing_instance_id > 0 and
            self.editing_mask is not None and
            0 <= y < self.editing_mask.shape[0] and
            0 <= x < self.editing_mask.shape[1] and
            self.editing_mask[y, x] > 0
        ):
            return self.editing_instance_id, (
                self.editing_instance_category or
                self.current_annotation_type
            )

        instance_id, category = self._find_instance_entry_at_point(x, y)
        if instance_id > 0:
            return instance_id, category

        if self.show_bboxes:
            bbox_id = self._get_bbox_at_pos(scene_pos)
            if bbox_id:
                return bbox_id, self.annotation_metadata.get(
                    bbox_id, {}
                ).get('category', self.current_annotation_type)

        return 0, None

    def _begin_pending_instance_switch_tap(self, instance_id, category, scene_pos, event):
        self._pending_switch_tap = {
            'instance_id': instance_id,
            'category': category,
            'scene_pos': QPointF(scene_pos),
            'view_pos': self._event_view_pos(event),
        }

    def _pending_switch_tap_moved(self, event):
        if not self._pending_switch_tap:
            return False
        start_pos = self._pending_switch_tap['view_pos']
        current_pos = self._event_view_pos(event)
        dx = current_pos.x() - start_pos.x()
        dy = current_pos.y() - start_pos.y()
        return (dx * dx + dy * dy) ** 0.5 > self._tap_move_tolerance_px

    def _complete_instance_switch_tap(self, instance_id, category):
        now = time.monotonic()
        candidate = (instance_id, category)
        if (
            self._switch_tap_candidate != candidate or
            now - self._switch_tap_last_time > self._switch_tap_max_interval_seconds
        ):
            self._switch_tap_candidate = candidate
            self._switch_tap_count = 0

        self._switch_tap_count += 1
        self._switch_tap_last_time = now
        remaining = self._switch_tap_required - self._switch_tap_count

        if remaining <= 0:
            self._reset_instance_switch_tap_sequence()
            self.start_editing_instance(instance_id, category=category)
            self.instance_switch_tap_progress.emit(instance_id, category, 0)
            return True

        self.instance_switch_tap_progress.emit(instance_id, category, remaining)
        return False

    def _start_brush_stroke_at(self, scene_pos):
        active_id, _ = self._active_brush_instance()
        if active_id <= 0:
            return False

        if not self._can_start_brush_stroke_at(scene_pos):
            return False

        if self.editing_instance_id <= 0 and self.selected_mask_idx > 0:
            self.start_editing_instance(
                self.selected_mask_idx,
                category=self.selected_instance_category or self.current_annotation_type
            )

        self._begin_brush_history_step()
        self.is_drawing = True
        self.last_point = scene_pos
        return True
        
    def load_image(self, image_path_or_array, preserve_view=False):
        """Load an image file or numpy array.

        Args:
            image_path_or_array: Path or numpy image array to display.
            preserve_view: Keep the current zoom/pan transform after replacing
                the frame image. Used for frame-to-frame navigation.
        """
        view_state = self.get_view_state() if preserve_view else None
        self._cancel_pending_instance_switch_tap(reset_sequence=True)

        # Explicitly clear old resources before loading new image
        if self.image_item:
            try:
                if self.image_item.scene() == self.scene:
                    self.scene.removeItem(self.image_item)
            except (RuntimeError, AttributeError):
                # Item already deleted by Qt or invalid
                pass
            self.image_item = None
        
        # Clear all visualization items
        for item in self.mask_items:
            try:
                if item.scene() == self.scene:
                    self.scene.removeItem(item)
            except (RuntimeError, AttributeError):
                # Item already deleted by Qt or invalid
                pass
        self.mask_items.clear()
        
        for item in self.label_items:
            try:
                if item.scene() == self.scene:
                    self.scene.removeItem(item)
            except (RuntimeError, AttributeError):
                # Item already deleted by Qt or invalid
                pass
        self.label_items.clear()

        for item in self.aruco_label_items:
            try:
                if item.scene() == self.scene:
                    self.scene.removeItem(item)
            except (RuntimeError, AttributeError):
                # Item already deleted by Qt or invalid
                pass
        self.aruco_label_items.clear()
        
        for item in self.prompt_items:
            try:
                if item.scene() == self.scene:
                    self.scene.removeItem(item)
            except (RuntimeError, AttributeError):
                # Item already deleted by Qt or invalid
                pass
        self.prompt_items.clear()
        
        if self.selection_border_item:
            try:
                if self.selection_border_item.scene() == self.scene:
                    self.scene.removeItem(self.selection_border_item)
            except (RuntimeError, AttributeError):
                # Item already deleted by Qt or invalid
                pass
            self.selection_border_item = None
        
        if self.editing_mask_item:
            try:
                if self.editing_mask_item.scene() == self.scene:
                    self.scene.removeItem(self.editing_mask_item)
            except (RuntimeError, AttributeError):
                # Item already deleted by Qt or invalid
                pass
            self.editing_mask_item = None
        self._remove_editing_bbox_item()
        
        # Clear cached overlays to free memory
        self._cached_overlay = None
        self._editing_overlay_cache = None
        
        # Delete old image data
        old_image = self.current_image
        self.current_image = None
        del old_image
        
        # Check if input is a numpy array
        if isinstance(image_path_or_array, np.ndarray):
            # Already a numpy array (grayscale H×W or RGB H×W×3)
            self.current_image = image_path_or_array
            self.image_path = None
        else:
            # It's a file path
            self.image_path = Path(image_path_or_array)
            
            # Load as grayscale to save memory (3× reduction)
            self.current_image = cv2.imread(str(image_path_or_array), cv2.IMREAD_GRAYSCALE)
            
            # Check if image was loaded successfully
            if self.current_image is None or self.current_image.size == 0:
                from PyQt6.QtWidgets import QMessageBox
                QMessageBox.warning(None, "Error", f"Failed to load image: {image_path_or_array}")
                return
        
        # Convert to QPixmap for display
        # If grayscale (H×W), convert to RGB (H×W×3) for display only
        if len(self.current_image.shape) == 2:
            # Grayscale: convert to RGB for display
            h, w = self.current_image.shape
            display_image = cv2.cvtColor(self.current_image, cv2.COLOR_GRAY2RGB)
            bytes_per_line = 3 * w
            # Convert display image to bytes and immediately create QImage, then pixmap
            # Using tobytes() creates a copy, so QImage doesn't reference display_image
            pixmap = QPixmap.fromImage(
                QImage(display_image.tobytes(), w, h, bytes_per_line, QImage.Format.Format_RGB888).copy()
            )
            # Explicitly delete temporary display image
            del display_image
        else:
            # Already RGB
            h, w, ch = self.current_image.shape
            bytes_per_line = ch * w
            pixmap = QPixmap.fromImage(
                QImage(self.current_image.tobytes(), w, h, bytes_per_line, QImage.Format.Format_RGB888).copy()
            )
        
        # Clear remaining scene items
        self._clear_brush_cursor_preview()
        self.scene.clear()
        self.bee_mask = None
        self.chamber_mask = None
        self.hive_mask = None
        self.pollen_mask = None
        self.next_mask_id = 1
        self.mask_colors = {}
        self.positive_points = []
        self.negative_points = []
        self.active_sam2_mask_idx = -1
        self.selected_instance_category = None
        self.editing_instance_category = None
        self.editing_bbox_item = None
        self.hidden_instance_keys.clear()
        self.hidden_bbox_instance_ids.clear()
        self.aruco_instance_labels = {}
        self._clear_brush_history()
        
        # Add image to scene
        self.image_item = QGraphicsPixmapItem(pixmap)
        self.scene.addItem(self.image_item)
        
        self._view_restore_token += 1
        view_token = self._view_restore_token
        from PyQt6.QtCore import QTimer
        QTimer.singleShot(
            0,
            lambda token=view_token, state=view_state: self._apply_initial_view(token, state)
        )
    
    def clear_image(self):
        """Clear the current image and all annotations"""
        self._clear_brush_cursor_preview()
        self.scene.clear()
        self.current_image = None
        self.image_path = None
        self.image_item = None
        self.bee_mask = None
        self.chamber_mask = None
        self.hive_mask = None
        self.pollen_mask = None
        self.next_mask_id = 1
        self.mask_items = []  # QGraphicsPixmapItems for visualization 
        self.mask_colors = {}  # Dict mapping instance_id -> (r,g,b) color
        self.label_items = []  # Clear instance label items
        self.aruco_label_items = []
        self.aruco_instance_labels = {}
        self.positive_points = []
        self.negative_points = []
        self.prompt_items = []
        self.active_sam2_mask_idx = -1
        self.selection_border_item = None
        self.selected_instance_category = None
        self.editing_instance_category = None
        self.editing_bbox_item = None
        self.hidden_instance_keys.clear()
        self.hidden_bbox_instance_ids.clear()
        self._clear_brush_history()
        
    def fit_to_window(self):
        """Fit image to window"""
        if self.image_item:
            self.fitInView(self.image_item, Qt.AspectRatioMode.KeepAspectRatio)
            self.zoom_factor = 1.0

    def _apply_initial_view(self, token, view_state):
        """Apply the queued fit/restore for the latest loaded image only."""
        if token != self._view_restore_token:
            return
        if view_state is not None:
            self.restore_view_state(view_state)
        else:
            self.fit_to_window()

    def get_view_state(self):
        """Capture current zoom and pan state."""
        viewport_center = self.viewport().rect().center()
        return {
            'transform': QTransform(self.transform()),
            'center': QPointF(self.mapToScene(viewport_center)),
            'h_scroll': self.horizontalScrollBar().value(),
            'v_scroll': self.verticalScrollBar().value(),
            'zoom_factor': self.zoom_factor,
        }

    def restore_view_state(self, state):
        """Restore a zoom and pan state captured before frame navigation."""
        if not state or not self.image_item:
            return

        self.setTransform(state['transform'])
        if 'center' in state:
            self.centerOn(state['center'])
        else:
            self.horizontalScrollBar().setValue(state['h_scroll'])
            self.verticalScrollBar().setValue(state['v_scroll'])
        self.zoom_factor = state.get('zoom_factor', self.transform().m11())
            
    def resizeEvent(self, event):
        """Handle resize event to fit image to new window size"""
        super().resizeEvent(event)
        self._position_brush_size_overlay()
        if self.image_item and self.zoom_factor == 1.0:
            # Only auto-fit if not zoomed in/out
            self.fit_to_window()
            
    def zoom_in(self):
        """Zoom in"""
        self.scale(1.1, 1.1)
        self.zoom_factor *= 1.1
        
    def zoom_out(self):
        """Zoom out with limit to prevent zooming smaller than fit-to-window"""
        if not self.image_item:
            return
        
        # Get current transform scale
        current_transform = self.transform()
        current_scale = current_transform.m11()
        
        # Calculate what scale we'd need to fit the whole image
        viewport_rect = self.viewport().rect()
        image_rect = self.image_item.boundingRect()
        
        scale_x = viewport_rect.width() / image_rect.width()
        scale_y = viewport_rect.height() / image_rect.height()
        min_scale = min(scale_x, scale_y)
        
        # Only zoom out if we're more zoomed in than fit-to-window
        if current_scale > min_scale * 0.95:  # 0.95 adds small tolerance
            self.scale(1/1.1, 1/1.1)
            self.zoom_factor /= 1.1
        
    def wheelEvent(self, event):
        """Handle mouse wheel for zooming"""
        if event.angleDelta().y() > 0:
            self.zoom_in()
        else:
            self.zoom_out()
            
    def set_tool(self, tool_name):
        """Set current tool"""
        # Flush any pending visualization updates when switching tools
        if self._viz_update_timer.isActive():
            self._viz_update_timer.stop()
            self._flush_pending_viz_update()
        self._cancel_pending_instance_switch_tap(reset_sequence=True)
        
        self.current_tool = tool_name
        
        if tool_name == 'pan':
            self.setDragMode(QGraphicsView.DragMode.ScrollHandDrag)
            self.setCursor(Qt.CursorShape.OpenHandCursor)
        else:
            self.setDragMode(QGraphicsView.DragMode.NoDrag)
            
            if tool_name == 'sam2_prompt':
                self.setCursor(Qt.CursorShape.CrossCursor)
            elif tool_name == 'brush':
                self.setCursor(Qt.CursorShape.CrossCursor)
            elif tool_name == 'eraser':
                self.setCursor(Qt.CursorShape.CrossCursor)
            elif tool_name == 'sam2_box':
                self.setCursor(Qt.CursorShape.CrossCursor)
            elif tool_name == 'inference_box':
                self.setCursor(Qt.CursorShape.CrossCursor)
        self._update_brush_cursor_preview()

    def mouseDoubleClickEvent(self, event):
        """Treat brush/eraser double-clicks as regular taps for switch counting."""
        if (
            self.current_tool in {'brush', 'eraser'} and
            event.button() == Qt.MouseButton.LeftButton
        ):
            self.mousePressEvent(event)
            return
        super().mouseDoubleClickEvent(event)
                
    def mousePressEvent(self, event):
        """Handle mouse press"""
        if self.current_image is None:
            return

        if event.button() == Qt.MouseButton.MiddleButton:
            if self.current_tool in {'brush', 'eraser'}:
                self.brush_eraser_toggle_requested.emit()
                event.accept()
                return
        
        # Handle right-click for context menu (category change). Avoid
        # intercepting active annotation tools: SAM2 uses right-click for
        # negative prompts, and brush/eraser right-clicks are easy accidents.
        if event.button() == Qt.MouseButton.RightButton:
            if self.current_tool == 'sam2_prompt':
                pass
            elif self.is_drawing:
                event.accept()
                return
            elif self.current_tool in {'brush', 'eraser'}:
                self._show_brush_context_menu(event)
                return
            elif self.current_tool == 'bbox':
                self._show_category_context_menu(event, allow_active_fallback=True)
                return
            else:
                self._show_category_context_menu(event)
                return
            
        # Get scene position
        scene_pos = self.mapToScene(event.pos())
        
        if self.current_tool == 'sam2_prompt':
            # SAM2 point prompting
            x, y = int(scene_pos.x()), int(scene_pos.y())
            is_positive = event.button() == Qt.MouseButton.LeftButton
            
            # Store and visualize the point
            if is_positive:
                self.positive_points.append((x, y))
            else:
                self.negative_points.append((x, y))
            self.add_prompt_marker(x, y, is_positive)
            
            # Emit signal for SAM2 prediction
            self.point_clicked.emit(x, y, is_positive)
            
        elif self.current_tool == 'sam2_box':
            # Start drawing box
            if event.button() == Qt.MouseButton.LeftButton:
                self.is_drawing = True
                self.drawing_start = scene_pos
                
        elif self.current_tool == 'inference_box':
            # Check if clicking on inference box or its handles
            if event.button() == Qt.MouseButton.LeftButton:
                if self.inference_box_rect:
                    # Check if clicking on a handle
                    handle = self._get_handle_at_pos(scene_pos)
                    if handle:
                        self.dragging_box = True
                        self.dragging_handle = handle
                        self.drag_offset = scene_pos
                    elif self.inference_box_rect.contains(scene_pos):
                        # Clicking inside box - move it
                        self.dragging_box = True
                        self.dragging_handle = 'move'
                        self.drag_offset = QPointF(scene_pos.x() - self.inference_box_rect.x(),
                                                   scene_pos.y() - self.inference_box_rect.y())
                    else:
                        # Start drawing new box
                        self.is_drawing = True
                        self.drawing_start = scene_pos
                        self.clear_inference_box()
                else:
                    # Start drawing new box
                    self.is_drawing = True
                    self.drawing_start = scene_pos
                
        elif self.current_tool == 'brush' or self.current_tool == 'eraser':
            # Start drawing/erasing
            if event.button() == Qt.MouseButton.LeftButton:
                clicked_instance_id, clicked_category = self._brush_switch_target_at(scene_pos)

                if (clicked_instance_id > 0 and
                    not self._is_active_brush_instance(clicked_instance_id, clicked_category)):
                    self._begin_pending_instance_switch_tap(
                        clicked_instance_id,
                        clicked_category,
                        scene_pos,
                        event
                    )
                    event.accept()
                    return

                self._reset_instance_switch_tap_sequence()
                self._start_brush_stroke_at(scene_pos)
                event.accept()
                return
        
        elif self.current_tool == 'bbox':
            if event.button() == Qt.MouseButton.LeftButton:
                # If there's a selected instance, check if it already has a bbox
                if self.selected_mask_idx > 0:
                    if self.selected_mask_idx in self.bbox_items_map:
                        # Instance has a bbox - interact with it only if the
                        # click lands on this bbox or one of its handles.
                        # Check if clicking on handle
                        handle = self._get_bbox_handle_at_pos(scene_pos)
                        if handle:
                            self._select_bbox(self.selected_mask_idx, zoom=False)
                            self.dragging_bbox_handle = handle
                            self.dragging_bbox = True
                            self.bbox_drag_offset = scene_pos
                            return
                        elif self.bbox_items_map[self.selected_mask_idx].rect().contains(scene_pos):
                            self._select_bbox(self.selected_mask_idx, zoom=False)
                            # Clicking inside bbox - start dragging
                            self.dragging_bbox = True
                            self.dragging_bbox_handle = 'move'
                            rect = self.bbox_items_map[self.selected_mask_idx].rect()
                            self.bbox_drag_offset = QPointF(scene_pos.x() - rect.x(), scene_pos.y() - rect.y())
                            return

                        bbox_id = self._get_bbox_at_pos(scene_pos)
                        if bbox_id and bbox_id != self.selected_mask_idx:
                            self._select_bbox(bbox_id, zoom=False)
                            self.dragging_bbox = True
                            self.dragging_bbox_handle = 'move'
                            rect = self.bbox_items_map[bbox_id].rect()
                            self.bbox_drag_offset = QPointF(scene_pos.x() - rect.x(), scene_pos.y() - rect.y())
                            return

                        event.accept()
                        return
                    else:
                        # Instance doesn't have a bbox yet - allow drawing one
                        self.is_drawing = True
                        self.drawing_new_bbox = True
                        self.drawing_start = scene_pos
                        return
                
                # No selected instance - check general bbox interactions
                # Check if clicking on existing bbox handle
                if self.selected_bbox_id:
                    handle = self._get_bbox_handle_at_pos(scene_pos)
                    if handle:
                        self.dragging_bbox_handle = handle
                        self.dragging_bbox = True
                        self.bbox_drag_offset = scene_pos
                        return
                
                # Check if clicking inside existing bbox
                bbox_id = self._get_bbox_at_pos(scene_pos)
                if bbox_id:
                    self._select_bbox(bbox_id, zoom=False)
                    self.dragging_bbox = True
                    self.dragging_bbox_handle = 'move'
                    rect = self.bbox_items_map[bbox_id].rect()
                    self.bbox_drag_offset = QPointF(scene_pos.x() - rect.x(), scene_pos.y() - rect.y())
                else:
                    event.accept()
                    return

        elif self.current_tool == 'pan':
            if event.button() == Qt.MouseButton.LeftButton:
                self._select_instance_or_bbox_at_scene_pos(scene_pos)
            super().mousePressEvent(event)
                
        else:
            if event.button() == Qt.MouseButton.LeftButton:
                if self._select_instance_or_bbox_at_scene_pos(scene_pos):
                    event.accept()
                    return
            super().mousePressEvent(event)

    def _find_instance_entry_at_point(self, x, y, category=None):
        """Find the visible instance and category at an image coordinate."""
        if not self._segmentation_overlays_visible():
            return 0, None

        if category in ('bee', 'chamber', 'hive', 'pollen'):
            masks_to_search = [(category, self._get_mask_array_by_category(category))]
        else:
            # Search in approximate visual top-to-bottom order.
            masks_to_search = [
                ('bee', self.bee_mask),
                ('pollen', self.pollen_mask),
                ('hive', self.hive_mask),
                ('chamber', self.chamber_mask),
            ]

        for candidate_category, mask_array in masks_to_search:
            if not self.annotation_type_visibility.get(candidate_category, True):
                continue
            if mask_array is None:
                continue
            if not (0 <= x < mask_array.shape[1] and 0 <= y < mask_array.shape[0]):
                continue
            instance_id = int(mask_array[y, x])
            if instance_id > 0 and self.is_instance_enabled(instance_id, candidate_category):
                return instance_id, candidate_category

        return 0, None

    def _select_instance_or_bbox_at_scene_pos(self, scene_pos):
        """Select a visible instance or bbox under the cursor."""
        x, y = int(scene_pos.x()), int(scene_pos.y())
        instance_id, category = self._find_instance_entry_at_point(x, y)

        if instance_id > 0:
            self.highlight_instance(instance_id, category=category)
            return True

        bbox_id = self._get_bbox_at_pos(scene_pos)
        if bbox_id:
            self._select_bbox(bbox_id, zoom=False)
            return True

        return False
    
    def _context_instance_at_scene_pos(self, scene_pos, allow_active_fallback=False):
        """Return the best category-aware instance target for a context menu."""
        x, y = int(scene_pos.x()), int(scene_pos.y())

        instance_id, category = self._find_instance_entry_at_point(x, y)
        if instance_id > 0:
            return int(instance_id), category

        bbox_id = self._get_bbox_at_pos(scene_pos)
        if bbox_id:
            category = self.annotation_metadata.get(bbox_id, {}).get(
                'category',
                self.selected_instance_category or self.current_annotation_type
            )
            return int(bbox_id), category

        if not allow_active_fallback:
            return 0, None

        if self.editing_instance_id > 0:
            category = self.editing_instance_category or self.annotation_metadata.get(
                self.editing_instance_id, {}
            ).get('category', self.current_annotation_type)
            return int(self.editing_instance_id), category

        if self.selected_mask_idx > 0:
            category = self.selected_instance_category or self.annotation_metadata.get(
                self.selected_mask_idx, {}
            ).get('category', self.current_annotation_type)
            return int(self.selected_mask_idx), category

        return 0, None

    def _add_change_category_menu(self, menu, instance_id, current_category):
        """Add the shared change-category submenu for one instance."""
        if instance_id <= 0:
            disabled_action = QAction("Change Instance Category (select an instance first)", self)
            disabled_action.setEnabled(False)
            menu.addAction(disabled_action)
            return

        change_category_menu = menu.addMenu(f"Change Instance {instance_id} Category")

        categories = [
            ('bee', 'Bee (per-frame)'),
            ('hive', 'Hive (video-level)'),
            ('chamber', 'Chamber (video-level)'),
            ('pollen', 'Pollen (video-level)')
        ]

        current_category = current_category or self.annotation_metadata.get(
            instance_id, {}
        ).get('category', 'bee')

        for cat_id, cat_label in categories:
            if cat_id != current_category:
                action = QAction(cat_label, self)
                action.triggered.connect(
                    lambda checked=False, iid=instance_id, old_cat=current_category, new_cat=cat_id:
                    self.change_instance_category(iid, new_cat, old_category=old_cat)
                )
                change_category_menu.addAction(action)
            else:
                action = QAction(f"{cat_label} (current)", self)
                action.setEnabled(False)
                change_category_menu.addAction(action)

    def _category_label(self, category):
        labels = {
            'bee': 'Bee',
            'hive': 'Hive',
            'chamber': 'Chamber',
            'pollen': 'Pollen',
        }
        return labels.get(category, 'Bee')

    def _preferred_new_instance_category(self, context_category=None):
        """Choose the category for context-menu same-kind creation."""
        for category in (
            context_category,
            self.editing_instance_category,
            self.selected_instance_category,
            self.current_annotation_type,
        ):
            if category in {'bee', 'hive', 'chamber', 'pollen'}:
                return category
        return 'bee'

    def _add_new_instance_context_actions(self, menu, context_category=None):
        """Add context-menu actions for creating a new instance."""
        same_category = self._preferred_new_instance_category(context_category)
        same_label = self._category_label(same_category)

        menu.addSeparator()

        same_kind_action = QAction(f"New Instance of Same Kind ({same_label})", self)
        same_kind_action.triggered.connect(
            lambda checked=False, category=same_category:
            self.new_instance_requested.emit(category)
        )
        menu.addAction(same_kind_action)

        new_instance_menu = menu.addMenu("New Instance")
        for category in ('bee', 'hive', 'chamber', 'pollen'):
            action = QAction(self._category_label(category), self)
            action.triggered.connect(
                lambda checked=False, cat=category:
                self.new_instance_requested.emit(cat)
            )
            new_instance_menu.addAction(action)

    def _show_category_context_menu(self, event, allow_active_fallback=False):
        """Show context menu for changing categories and creating instances."""
        # Get scene position
        scene_pos = self.mapToScene(event.pos())

        # Find which instance (if any) is at this position
        instance_id, current_category = self._context_instance_at_scene_pos(
            scene_pos,
            allow_active_fallback=allow_active_fallback
        )

        menu = QMenu(self)

        if instance_id > 0:
            self._add_change_category_menu(menu, instance_id, current_category)
        else:
            current_category = self._preferred_new_instance_category(current_category)

        self._add_new_instance_context_actions(menu, current_category)
        self._add_split_hovered_segment_action(menu, scene_pos)
        menu.exec(self._event_global_pos(event))

    def _event_global_pos(self, event):
        """Return the global cursor position for PyQt6/PyQt5 mouse events."""
        if hasattr(event, 'globalPosition'):
            return event.globalPosition().toPoint()
        return event.globalPos()

    def _next_available_instance_id(self):
        """Return a globally unused instance ID and advance next_mask_id."""
        self._cached_instance_ids = None
        existing_ids = set(self.get_instance_ids())
        new_id = max(1, int(self.next_mask_id))
        while new_id in existing_ids:
            new_id += 1
        self.next_mask_id = new_id + 1
        return new_id

    def _split_source_at_scene_pos(self, scene_pos):
        """Find a split-capable segmentation source at a scene position."""
        x, y = int(scene_pos.x()), int(scene_pos.y())

        if self.editing_instance_id > 0 and self.editing_mask is not None:
            if (
                0 <= y < self.editing_mask.shape[0] and
                0 <= x < self.editing_mask.shape[1] and
                self.editing_mask[y, x] > 0 and
                self._is_editing_mask_visible()
            ):
                category = self.editing_instance_category or self.annotation_metadata.get(
                    self.editing_instance_id, {}
                ).get('category', self.current_annotation_type)
                return {
                    'kind': 'editing',
                    'x': x,
                    'y': y,
                    'instance_id': int(self.editing_instance_id),
                    'category': category,
                    'mask_pixels': self.editing_mask > 0,
                }

        instance_id, category = self._find_instance_entry_at_point(x, y)
        if instance_id <= 0:
            return None

        mask_array, category = self._get_mask_array_for_instance(instance_id, category)
        if mask_array is None:
            return None
        if not (0 <= y < mask_array.shape[0] and 0 <= x < mask_array.shape[1]):
            return None
        if int(mask_array[y, x]) != int(instance_id):
            return None

        return {
            'kind': 'mask',
            'x': x,
            'y': y,
            'instance_id': int(instance_id),
            'category': category,
            'mask_array': mask_array,
            'mask_pixels': mask_array == int(instance_id),
        }

    def _hovered_segment_split_info(self, scene_pos):
        """Return split info for the connected component under the cursor."""
        source = self._split_source_at_scene_pos(scene_pos)
        if source is None:
            return None, "hover over a mask first"

        mask_pixels = source['mask_pixels']
        if mask_pixels is None or not np.any(mask_pixels):
            return None, "hover over a mask first"

        x, y = source['x'], source['y']
        binary = mask_pixels.astype(np.uint8)
        if not (0 <= y < binary.shape[0] and 0 <= x < binary.shape[1]):
            return None, "hover over a mask first"
        if binary[y, x] == 0:
            return None, "hover over a mask first"

        _num_labels, labels = cv2.connectedComponents(binary, connectivity=8)
        component_label = int(labels[y, x])
        if component_label <= 0:
            return None, "hover over a mask first"

        component_pixels = labels == component_label
        remaining_pixels = mask_pixels & ~component_pixels
        if not np.any(remaining_pixels):
            return None, "only one connected piece found"

        source['component_pixels'] = component_pixels
        source['remaining_pixels'] = remaining_pixels
        return source, None

    def _update_metadata_for_segment(self, instance_id, category, pixels):
        """Update basic metadata for a mask-backed segment."""
        if instance_id not in self.annotation_metadata:
            self.annotation_metadata[instance_id] = {}

        metadata = self.annotation_metadata[instance_id]
        metadata['mask_id'] = int(instance_id)
        metadata['category'] = category
        metadata.pop('bbox_only', None)

        bbox = self.get_mask_bbox(pixels.astype(np.uint8) * 255)
        if bbox is not None:
            metadata['bbox'] = [int(value) for value in bbox]

    def _metadata_for_split_instance(self, old_instance_id, new_instance_id, category, pixels):
        """Build metadata for a split-off instance without copying identity markers."""
        old_metadata = self.annotation_metadata.get(old_instance_id, {})
        skip_keys = {
            'marker', 'bbox', 'bbox_only', 'mask', 'mask_rle', 'mask_id',
            'instance_id', 'id', 'label', 'area'
        }
        metadata = {
            key: value
            for key, value in old_metadata.items()
            if key not in skip_keys
        }
        metadata['mask_id'] = int(new_instance_id)
        metadata['label'] = f'instance_{new_instance_id}'
        metadata['category'] = category

        bbox = self.get_mask_bbox(pixels.astype(np.uint8) * 255)
        if bbox is not None:
            metadata['bbox'] = [int(value) for value in bbox]

        return metadata

    def split_hovered_segment_at(self, scene_pos):
        """Move the connected segment under the cursor to a new same-class ID."""
        split_info, reason = self._hovered_segment_split_info(scene_pos)
        if split_info is None:
            print(f"Split hovered segment skipped: {reason}")
            return False

        old_id = split_info['instance_id']
        category = split_info['category']
        component_pixels = split_info['component_pixels']
        remaining_pixels = split_info['remaining_pixels']
        new_id = self._next_available_instance_id()

        if split_info['kind'] == 'editing':
            self.editing_mask[component_pixels] = 0
            mask_array = self._get_mask_array_by_category(category)
            if mask_array is None:
                mask_array = np.zeros(self.editing_mask.shape, dtype=np.int32)
                self._set_mask_array_by_category(category, mask_array)
            mask_array[component_pixels] = new_id
        else:
            mask_array = split_info['mask_array']
            mask_array[component_pixels] = new_id

        self._update_metadata_for_segment(old_id, category, remaining_pixels)
        self.annotation_metadata[new_id] = self._metadata_for_split_instance(
            old_id,
            new_id,
            category,
            component_pixels
        )
        self.ensure_instance_color(new_id, category)
        self.set_instance_visible(new_id, category, True, rebuild=False)

        self._combined_mask_dirty = True
        self._cached_instance_ids = None
        self._cached_bboxes.clear()
        self._cached_overlay = None
        self._clear_brush_history()

        self.selected_mask_idx = new_id
        self.selected_instance_category = category
        self.selected_bbox_id = None

        if self.current_tool in {'brush', 'eraser'}:
            self.start_editing_instance(new_id, category=category)
        else:
            self.rebuild_visualizations()
            self.set_selected_instance(new_id, category=category, zoom=False)
            self.instance_selected.emit(new_id, category)

        self.annotation_changed.emit()
        print(
            f"Split {category} instance {old_id}: "
            f"hovered segment is now instance {new_id}"
        )
        return True

    def _add_split_hovered_segment_action(self, menu, scene_pos):
        """Add a context-menu action for assigning the hovered segment a new ID."""
        split_info, reason = self._hovered_segment_split_info(scene_pos)

        menu.addSeparator()
        action = QAction("Split Hovered Segment Into New Instance", self)
        action.setEnabled(split_info is not None)
        if split_info is None:
            action.setText(f"Split Hovered Segment ({reason})")
        action.triggered.connect(
            lambda checked=False, pos=QPointF(scene_pos):
            self.split_hovered_segment_at(pos)
        )
        menu.addAction(action)

    def _show_brush_context_menu(self, event):
        """Show brush-specific actions at the cursor."""
        scene_pos = self.mapToScene(event.pos())
        menu = QMenu(self)
        instance_id, current_category = self._context_instance_at_scene_pos(
            scene_pos,
            allow_active_fallback=True
        )

        fill_action = QAction("Fill Enclosed Area", self)
        fill_action.setEnabled(
            self.editing_mask is not None and self.editing_instance_id > 0
        )
        fill_action.triggered.connect(
            lambda checked=False, pos=QPointF(scene_pos): self.fill_enclosed_region_at(pos)
        )
        menu.addAction(fill_action)

        if not fill_action.isEnabled():
            fill_action.setText("Fill Enclosed Area (select/draw an instance first)")

        subtract_action = QAction("Subtract Filled Area", self)
        subtract_action.setEnabled(
            self.editing_mask is not None and self.editing_instance_id > 0
        )
        subtract_action.triggered.connect(
            lambda checked=False, pos=QPointF(scene_pos): self.subtract_enclosed_region_at(pos)
        )
        menu.addAction(subtract_action)

        if not subtract_action.isEnabled():
            subtract_action.setText("Subtract Filled Area (select/draw an instance first)")

        self._add_new_instance_context_actions(menu, current_category)

        menu.addSeparator()
        self._add_change_category_menu(menu, instance_id, current_category)

        menu.addSeparator()

        undo_action = QAction("Undo Brush Step", self)
        undo_action.setEnabled(bool(self.brush_undo_stack))
        undo_action.triggered.connect(lambda checked=False: self.undo())
        menu.addAction(undo_action)

        redo_action = QAction("Redo Brush Step", self)
        redo_action.setEnabled(bool(self.brush_redo_stack))
        redo_action.triggered.connect(lambda checked=False: self.redo())
        menu.addAction(redo_action)

        self._add_split_hovered_segment_action(menu, scene_pos)

        menu.exec(self._event_global_pos(event))
            
    def mouseMoveEvent(self, event):
        """Handle mouse move"""
        if self.current_image is None:
            self._clear_brush_cursor_preview()
            return
            
        scene_pos = self.mapToScene(event.pos())
        self._update_brush_cursor_preview(scene_pos)

        if self._pending_switch_tap is not None:
            if self._pending_switch_tap_moved(event):
                press_scene_pos = self._pending_switch_tap['scene_pos']
                self._cancel_pending_instance_switch_tap(reset_sequence=True)
                active_id, _ = self._active_brush_instance()
                if active_id > 0 and self._start_brush_stroke_at(press_scene_pos):
                    self._is_actively_drawing = True
                    self.draw_on_mask(self.last_point, scene_pos)
                    self.last_point = scene_pos
            event.accept()
            return
        
        if self.is_drawing:
            if self.drawing_new_bbox:
                # Update bbox preview
                if self.temp_item:
                    try:
                        self.scene.removeItem(self.temp_item)
                    except RuntimeError:
                        pass
                
                # Draw temporary rectangle
                rect = QRectF(self.drawing_start, scene_pos).normalized()
                self.temp_item = OutwardBBoxRectItem(rect)
                pen = QPen(QColor(255, 255, 0))  # Yellow
                pen.setWidth(3)
                pen.setStyle(Qt.PenStyle.DashLine)
                self.temp_item.setPen(pen)
                self.temp_item.setZValue(150)
                self.scene.addItem(self.temp_item)
            elif self.current_tool == 'sam2_box':
                # Update box preview
                if self.temp_item:
                    try:
                        self.scene.removeItem(self.temp_item)
                    except RuntimeError:
                        pass
                    
                from PyQt6.QtWidgets import QGraphicsRectItem
                rect = QRectF(self.drawing_start, scene_pos).normalized()
                self.temp_item = QGraphicsRectItem(rect)
                self.temp_item.setPen(QPen(QColor(0, 255, 0), 2))
                self.scene.addItem(self.temp_item)
                
            elif self.current_tool == 'inference_box':
                # Update inference box preview
                if self.temp_item:
                    try:
                        self.scene.removeItem(self.temp_item)
                    except RuntimeError:
                        pass
                    
                from PyQt6.QtWidgets import QGraphicsRectItem
                rect = QRectF(self.drawing_start, scene_pos).normalized()
                self.temp_item = QGraphicsRectItem(rect)
                self.temp_item.setPen(QPen(QColor(255, 165, 0), 3))  # Orange
                self.temp_item.setZValue(1000)  # Draw on top
                self.scene.addItem(self.temp_item)
                
            elif self.current_tool == 'brush' or self.current_tool == 'eraser':
                # Draw/erase on mask
                self._is_actively_drawing = True
                self.draw_on_mask(self.last_point, scene_pos)
                self.last_point = scene_pos
                
        elif self.dragging_box and self.current_tool == 'inference_box':
            # Handle dragging/resizing inference box
            if self.dragging_handle == 'move':
                # Move entire box
                new_x = scene_pos.x() - self.drag_offset.x()
                new_y = scene_pos.y() - self.drag_offset.y()
                self.inference_box_rect.moveTo(new_x, new_y)
                self._update_inference_box_display()
            elif self.dragging_handle in ['tl', 'tr', 'bl', 'br']:
                # Resize from corner
                rect = self.inference_box_rect
                if self.dragging_handle == 'tl':
                    rect.setTopLeft(scene_pos)
                elif self.dragging_handle == 'tr':
                    rect.setTopRight(scene_pos)
                elif self.dragging_handle == 'bl':
                    rect.setBottomLeft(scene_pos)
                elif self.dragging_handle == 'br':
                    rect.setBottomRight(scene_pos)
                self.inference_box_rect = rect.normalized()
                self._update_inference_box_display()
        
        elif self.dragging_bbox and self.show_bboxes:
            # Handle dragging/resizing bbox
            if self.dragging_bbox_handle == 'move':
                # Move entire bbox
                if self.selected_bbox_id and self.selected_bbox_id in self.bbox_items_map:
                    rect_item = self.bbox_items_map[self.selected_bbox_id]
                    rect = rect_item.rect()
                    new_x = scene_pos.x() - self.bbox_drag_offset.x()
                    new_y = scene_pos.y() - self.bbox_drag_offset.y()
                    rect.moveTo(new_x, new_y)
                    rect_item.setRect(rect)
                    self._update_bbox_handles()
            elif self.dragging_bbox_handle in ['tl', 'tr', 'bl', 'br']:
                # Resize from corner
                if self.selected_bbox_id and self.selected_bbox_id in self.bbox_items_map:
                    rect_item = self.bbox_items_map[self.selected_bbox_id]
                    rect = rect_item.rect()
                    if self.dragging_bbox_handle == 'tl':
                        rect.setTopLeft(scene_pos)
                    elif self.dragging_bbox_handle == 'tr':
                        rect.setTopRight(scene_pos)
                    elif self.dragging_bbox_handle == 'bl':
                        rect.setBottomLeft(scene_pos)
                    elif self.dragging_bbox_handle == 'br':
                        rect.setBottomRight(scene_pos)
                    rect = rect.normalized()
                    rect_item.setRect(rect)
                    self._update_bbox_handles()
                
        else:
            super().mouseMoveEvent(event)

    def leaveEvent(self, event):
        """Hide brush footprint cursor when the mouse leaves the canvas."""
        self._clear_brush_cursor_preview()
        super().leaveEvent(event)
            
    def mouseReleaseEvent(self, event):
        """Handle mouse release"""
        if self.current_image is None:
            return

        if self._pending_switch_tap is not None:
            pending_tap = self._pending_switch_tap
            moved = self._pending_switch_tap_moved(event)
            self._cancel_pending_instance_switch_tap(reset_sequence=moved)
            if not moved:
                self._complete_instance_switch_tap(
                    pending_tap['instance_id'],
                    pending_tap['category']
                )
            event.accept()
            return
            
        scene_pos = self.mapToScene(event.pos())
        
        if self.is_drawing:
            if self.drawing_new_bbox:
                # Finish drawing new bbox
                if self.temp_item:
                    try:
                        self.scene.removeItem(self.temp_item)
                    except RuntimeError:
                        pass
                    self.temp_item = None
                
                # Create the new bbox
                rect = QRectF(self.drawing_start, scene_pos).normalized()
                
                # Only create if bbox has minimum size
                if rect.width() > 10 and rect.height() > 10:
                    self._create_new_bbox(rect)
                
                self.drawing_new_bbox = False
                self.is_drawing = False
                self.setToolTip("Click on a bbox to select and edit")
                
            elif self.current_tool == 'sam2_box':
                # Emit box drawn signal
                if self.temp_item:
                    try:
                        self.scene.removeItem(self.temp_item)
                    except RuntimeError:
                        pass
                    self.temp_item = None
                    
                x1, y1 = int(self.drawing_start.x()), int(self.drawing_start.y())
                x2, y2 = int(scene_pos.x()), int(scene_pos.y())
                self.box_drawn.emit(x1, y1, x2, y2)
                
            elif self.current_tool == 'inference_box':
                # Finish drawing inference box
                if self.temp_item:
                    try:
                        self.scene.removeItem(self.temp_item)
                    except RuntimeError:
                        pass
                    self.temp_item = None
                    
                # Store the box
                rect = QRectF(self.drawing_start, scene_pos).normalized()
                self.set_inference_box(rect)
                self.box_drawn.emit(int(rect.x()), int(rect.y()),
                                   int(rect.right()), int(rect.bottom()))
                
            elif self.current_tool == 'brush' or self.current_tool == 'eraser':
                # Flush any pending visualization updates before finishing
                if self._viz_update_timer.isActive():
                    self._viz_update_timer.stop()
                    self._flush_pending_viz_update()
                
                # Finish drawing - update border now
                self._is_actively_drawing = False
                self._finish_brush_history_step()
                deleted_empty_instance = False
                if (
                    self.editing_mask is not None and
                    not np.any(self.editing_mask > 0) and
                    self._should_delete_empty_editing_instance()
                ):
                    instance_id = self.editing_instance_id
                    category = self.editing_instance_category
                    self.delete_instance(instance_id, category=category)
                    deleted_empty_instance = True
                elif self.editing_mask is not None and self.selected_mask_idx == self.editing_instance_id:
                    self.add_selection_border(self.editing_mask)
                    self.update_instance_labels()
                
                # Emit annotation_changed now that drawing is complete
                if not deleted_empty_instance:
                    self.annotation_changed. emit()
                
            self.is_drawing = False
        elif self.dragging_box:
            # Finish dragging inference box
            self.dragging_box = False
            self.dragging_handle = None
            self.drag_offset = None
        elif self.dragging_bbox:
            # Finish dragging/resizing bbox
            if self.selected_bbox_id and self.selected_bbox_id in self.bbox_items_map:
                # Update the annotation metadata with new bbox coordinates
                rect_item = self.bbox_items_map[self.selected_bbox_id]
                rect = rect_item.rect()
                self.annotation_metadata[self.selected_bbox_id]['bbox'] = [
                    int(rect.x()), int(rect.y()), int(rect.width()), int(rect.height())
                ]
                # Update labels to reflect new position
                self.update_instance_labels()
                # Emit annotation changed signal
                self.annotation_changed.emit()
            self.dragging_bbox = False
            self.dragging_bbox_handle = None
            self.bbox_drag_offset = None
        else:
            super().mouseReleaseEvent(event)
    
    def set_inference_box(self, rect):
        """Set the inference box rectangle and display it with handles"""
        self.inference_box_rect = rect
        self._update_inference_box_display()
    
    def clear_inference_box(self):
        """Remove the inference box from display"""
        if self.inference_box_item:
            try:
                self.scene.removeItem(self.inference_box_item)
            except RuntimeError:
                pass
            self.inference_box_item = None
        for handle in self.inference_box_handles:
            try:
                self.scene.removeItem(handle)
            except RuntimeError:
                pass
        self.inference_box_handles = []
        self.inference_box_rect = None
    
    def get_inference_box(self):
        """Get the current inference box coordinates as (x1, y1, x2, y2)"""
        if self.inference_box_rect:
            return (int(self.inference_box_rect.x()), int(self.inference_box_rect.y()),
                    int(self.inference_box_rect.right()), int(self.inference_box_rect.bottom()))
        return None
    
    def _update_inference_box_display(self):
        """Update the visual display of the inference box with handles"""
        # Remove old graphics items
        if self.inference_box_item:
            try:
                self.scene.removeItem(self.inference_box_item)
            except RuntimeError:
                pass
        for handle in self.inference_box_handles:
            try:
                self.scene.removeItem(handle)
            except RuntimeError:
                pass
        self.inference_box_handles = []
        
        if not self.inference_box_rect:
            return
        
        from PyQt6.QtWidgets import QGraphicsRectItem, QGraphicsEllipseItem
        
        # Draw main box
        self.inference_box_item = QGraphicsRectItem(self.inference_box_rect)
        self.inference_box_item.setPen(QPen(QColor(255, 165, 0), 3))  # Orange
        self.inference_box_item.setZValue(1000)
        self.scene.addItem(self.inference_box_item)
        
        # Draw corner handles
        handle_size = 10
        corners = [
            ('tl', self.inference_box_rect.topLeft()),
            ('tr', self.inference_box_rect.topRight()),
            ('bl', self.inference_box_rect.bottomLeft()),
            ('br', self.inference_box_rect.bottomRight())
        ]
        
        for corner_name, point in corners:
            handle = QGraphicsEllipseItem(
                point.x() - handle_size/2, point.y() - handle_size/2,
                handle_size, handle_size
            )
            handle.setPen(QPen(QColor(255, 165, 0), 2))
            handle.setBrush(QBrush(QColor(255, 255, 255)))
            handle.setZValue(1001)
            handle.setData(0, corner_name)  # Store corner name in item data
            self.scene.addItem(handle)
            self.inference_box_handles.append(handle)
    
    def _get_handle_at_pos(self, pos):
        """Check if position is near a handle or inside box, return handle type"""
        if not self.inference_box_rect:
            return None
        
        handle_size = 15  # Slightly larger hit area
        
        # Check corners
        corners = {
            'tl': self.inference_box_rect.topLeft(),
            'tr': self.inference_box_rect.topRight(),
            'bl': self.inference_box_rect.bottomLeft(),
            'br': self.inference_box_rect.bottomRight()
        }
        
        for corner_name, point in corners.items():
            dx = abs(pos.x() - point.x())
            dy = abs(pos.y() - point.y())
            if dx < handle_size and dy < handle_size:
                return corner_name
        
        return None
            
    def draw_on_mask(self, start_pos, end_pos):
        """Draw on the current mask with brush/eraser"""
        # Initialize appropriate mask if needed
        if self.current_image is not None:
            h, w = self.current_image.shape[:2]
        else:
            return
        
        # Initialize appropriate mask based on annotation type
        if self.current_annotation_type == 'chamber':
            if self.chamber_mask is None:
                self.chamber_mask = np.zeros((h, w), dtype=np.int32)
        elif self.current_annotation_type == 'hive':
            if self.hive_mask is None:
                self.hive_mask = np.zeros((h, w), dtype=np.int32)
        elif self.current_annotation_type == 'pollen':
            if self.pollen_mask is None:
                self.pollen_mask = np.zeros((h, w), dtype=np.int32)
        else:  # bee
            if self.bee_mask is None:
                self.bee_mask = np.zeros((h, w), dtype=np.int32)

        if self.selected_mask_idx <= 0:
            return
        
        # Start editing mode if not already active
        was_tracking_brush_step = self._brush_step_active
        selected_category = self.selected_instance_category or self.current_annotation_type
        if (self.editing_instance_id != self.selected_mask_idx or
            self.editing_instance_category != selected_category):
            self.start_editing_instance(
                self.selected_mask_idx,
                category=selected_category
            )
            if was_tracking_brush_step:
                self._brush_step_active = True
                self._brush_step_start_snapshot = None
        self._capture_brush_step_start_if_needed()
            
        instance_id = self.selected_mask_idx
        
        # Convert to image coordinates
        x1, y1 = int(start_pos.x()), int(start_pos.y())
        x2, y2 = int(end_pos.x()), int(end_pos.y())
        
        # Draw directly on editing mask (much faster - no temp array allocation)
        if self.current_tool == 'brush':
            cv2.line(self.editing_mask, (x1, y1), (x2, y2), 255, self.brush_size)
        else:  # eraser
            cv2.line(self.editing_mask, (x1, y1), (x2, y2), 0, self.brush_size)
        self._cached_bboxes.pop(instance_id, None)
        
        # Calculate bounding box of stroke for dirty region (much more memory efficient)
        half_brush = self.brush_size // 2 + 1
        min_x = max(0, min(x1, x2) - half_brush)
        max_x = min(self.editing_mask.shape[1], max(x1, x2) + half_brush)
        min_y = max(0, min(y1, y2) - half_brush)
        max_y = min(self.editing_mask.shape[0], max(y1, y2) + half_brush)
        
        # Create dirty region mask for this stroke
        stroke_dirty = np.zeros(self.editing_mask.shape, dtype=bool)
        stroke_dirty[min_y:max_y, min_x:max_x] = True
        
        # Accumulate dirty pixels for batched update (major performance improvement)
        if self._dirty_pixels is None:
            self._dirty_pixels = stroke_dirty
        else:
            self._dirty_pixels |= stroke_dirty
        
        # Schedule throttled visualization update
        if not self._viz_update_timer.isActive():
            self._viz_update_timer.start()
        
        # Only emit annotation_changed for final stroke (on release) to avoid slowdown

    def fill_enclosed_region_at(self, scene_pos):
        """Fill the enclosed blank region under the cursor in the editing mask.

        The current instance's nonzero pixels act as the boundary. If the
        clicked blank region reaches the image edge, the outline is considered
        open and no fill is applied.
        """
        if self.current_image is None:
            return False

        if self._viz_update_timer.isActive():
            self._viz_update_timer.stop()
            self._flush_pending_viz_update()

        if self.editing_mask is None or self.editing_instance_id <= 0:
            print("Fill skipped: no active brush-edit instance.")
            return False

        x = int(scene_pos.x())
        y = int(scene_pos.y())
        h, w = self.editing_mask.shape
        if x < 0 or x >= w or y < 0 or y >= h:
            print("Fill skipped: click was outside the image.")
            return False

        if self.editing_mask[y, x] > 0:
            print("Fill skipped: click inside the unfilled enclosed area, not on the outline.")
            return False

        # Flood-fill the blank component under the cursor. Existing mask pixels
        # are boundaries; 128 marks the candidate fill region. Use 8-connected
        # fill so diagonal slivers along thick brush outlines are included.
        flood_source = self.editing_mask.copy()
        flood_source[flood_source > 0] = 255
        flood_mask = np.zeros((h + 2, w + 2), dtype=np.uint8)
        cv2.floodFill(flood_source, flood_mask, (x, y), 128, flags=8)
        fill_region = flood_source == 128

        if not np.any(fill_region):
            print("Fill skipped: no fillable region found.")
            return False

        touches_image_edge = (
            np.any(fill_region[0, :]) or
            np.any(fill_region[-1, :]) or
            np.any(fill_region[:, 0]) or
            np.any(fill_region[:, -1])
        )
        if touches_image_edge:
            print("Fill skipped: the clicked region is not fully enclosed.")
            return False

        self._push_current_brush_undo_snapshot()
        self.editing_mask[fill_region] = 255
        self._cached_bboxes.pop(self.editing_instance_id, None)
        self._update_editing_visualization_incremental(fill_region)

        if self.selected_mask_idx == self.editing_instance_id:
            self.add_selection_border(self.editing_mask)
            self.update_instance_labels()

        self.annotation_changed.emit()
        return True

    def subtract_enclosed_region_at(self, scene_pos):
        """Remove the enclosed filled region under the cursor from the editing mask.

        This is the inverse of fill_enclosed_region_at: blank pixels act as the
        boundary, so the user should erase/trace a closed outline inside the
        active mask, then right-click inside the filled area to subtract it.
        """
        if self.current_image is None:
            return False

        if self._viz_update_timer.isActive():
            self._viz_update_timer.stop()
            self._flush_pending_viz_update()

        if self.editing_mask is None or self.editing_instance_id <= 0:
            print("Subtract skipped: no active brush-edit instance.")
            return False

        x = int(scene_pos.x())
        y = int(scene_pos.y())
        h, w = self.editing_mask.shape
        if x < 0 or x >= w or y < 0 or y >= h:
            print("Subtract skipped: click was outside the image.")
            return False

        if self.editing_mask[y, x] == 0:
            print("Subtract skipped: click inside the filled area to remove, not on the erased outline.")
            return False

        # Flood-fill the filled component under the cursor. Blank pixels are
        # boundaries; 128 marks the candidate region to subtract. Use the same
        # 8-connected behavior as fill for diagonal brush-edge slivers.
        flood_source = self.editing_mask.copy()
        flood_source[flood_source > 0] = 255
        flood_mask = np.zeros((h + 2, w + 2), dtype=np.uint8)
        cv2.floodFill(flood_source, flood_mask, (x, y), 128, flags=8)
        subtract_region = flood_source == 128

        if not np.any(subtract_region):
            print("Subtract skipped: no filled region found.")
            return False

        touches_image_edge = (
            np.any(subtract_region[0, :]) or
            np.any(subtract_region[-1, :]) or
            np.any(subtract_region[:, 0]) or
            np.any(subtract_region[:, -1])
        )
        if touches_image_edge:
            print("Subtract skipped: the clicked region is not fully enclosed.")
            return False

        current_pixels = self.editing_mask > 0
        if not np.any(current_pixels & ~subtract_region):
            print("Subtract skipped: region would remove the entire active mask.")
            return False

        self._push_current_brush_undo_snapshot()
        self.editing_mask[subtract_region] = 0
        self._cached_bboxes.pop(self.editing_instance_id, None)
        self._update_editing_visualization_incremental(subtract_region)

        if self.selected_mask_idx == self.editing_instance_id:
            self.add_selection_border(self.editing_mask)
            self.update_instance_labels()

        self.annotation_changed.emit()
        return True

    def _clear_brush_history(self):
        """Clear brush/eraser undo history for the active edit session."""
        self.brush_undo_stack = []
        self.brush_redo_stack = []
        self._brush_step_active = False
        self._brush_step_start_snapshot = None

    def _make_editing_mask_snapshot(self):
        """Return a compressed binary snapshot of the active editing mask."""
        if self.editing_mask is None or self.editing_instance_id <= 0:
            return None

        binary_mask = self.editing_mask.reshape(-1) > 0
        return {
            'instance_id': self.editing_instance_id,
            'category': self.editing_instance_category,
            'shape': self.editing_mask.shape,
            'data': np.packbits(binary_mask)
        }

    def _snapshots_equal(self, first, second):
        """Compare two compressed editing-mask snapshots."""
        if first is None or second is None:
            return first is second
        return (
            first['instance_id'] == second['instance_id'] and
            first.get('category') == second.get('category') and
            first['shape'] == second['shape'] and
            np.array_equal(first['data'], second['data'])
        )

    def _restore_editing_mask_snapshot(self, snapshot):
        """Restore a compressed editing-mask snapshot."""
        if snapshot is None:
            return False

        instance_id = snapshot['instance_id']
        category = snapshot.get('category') or self.annotation_metadata.get(
            instance_id, {}
        ).get('category', self.current_annotation_type)
        shape = snapshot['shape']
        total_pixels = shape[0] * shape[1]
        restored = np.unpackbits(snapshot['data'], count=total_pixels)
        self.editing_mask = restored.reshape(shape).astype(np.uint8) * 255
        self.editing_instance_id = instance_id
        self.editing_instance_category = category
        self.selected_mask_idx = instance_id
        self.selected_instance_category = category
        self._cached_bboxes.pop(instance_id, None)
        self._dirty_pixels = None

        if self._editing_overlay_cache is not None:
            del self._editing_overlay_cache
            self._editing_overlay_cache = None

        self._update_editing_visualization()
        self.annotation_changed.emit()
        return True

    def _trim_brush_stack(self, stack):
        """Keep a brush history stack within the configured limit."""
        overflow = len(stack) - self.brush_history_limit
        if overflow > 0:
            del stack[:overflow]

    def _push_current_brush_undo_snapshot(self):
        """Push the current editing mask onto the undo stack."""
        snapshot = self._make_editing_mask_snapshot()
        if snapshot is None:
            return False
        self.brush_undo_stack.append(snapshot)
        self._trim_brush_stack(self.brush_undo_stack)
        self.brush_redo_stack.clear()
        return True

    def _begin_brush_history_step(self):
        """Start tracking one brush/eraser drag as a single undo step."""
        self._brush_step_active = True
        self._brush_step_start_snapshot = None
        self._capture_brush_step_start_if_needed()

    def _capture_brush_step_start_if_needed(self):
        """Capture the pre-edit mask once the editing mask exists."""
        if not self._brush_step_active or self._brush_step_start_snapshot is not None:
            return
        self._brush_step_start_snapshot = self._make_editing_mask_snapshot()

    def _finish_brush_history_step(self):
        """Finish the active brush/eraser drag and save it if it changed."""
        if not self._brush_step_active:
            return

        start_snapshot = self._brush_step_start_snapshot
        current_snapshot = self._make_editing_mask_snapshot()
        self._brush_step_active = False
        self._brush_step_start_snapshot = None

        if start_snapshot is None or current_snapshot is None:
            return
        if self._snapshots_equal(start_snapshot, current_snapshot):
            return

        self.brush_undo_stack.append(start_snapshot)
        self._trim_brush_stack(self.brush_undo_stack)
        self.brush_redo_stack.clear()
        
    def add_mask(self, mask, mask_id=None, color=None, rebuild_viz=True, category=None):
        """Add a new mask based on current annotation type
        
        Args:
            mask: Binary mask array (H×W with 0/255 or True/False values)
            mask_id: Optional persistent ID for bee masks (ignored for chamber/hive)
            color: Optional (r,g,b) color tuple. If None, uses default for type
            rebuild_viz: If True, rebuild visualization immediately
            category: Override annotation type ('bee', 'chamber', 'hive'). Defaults to current_annotation_type.
        """
        if not isinstance(mask, np.ndarray):
            return
        
        # Resolve effective category
        effective_category = category if category is not None else self.current_annotation_type

        # Convert binary mask to boolean
        binary_mask = mask > 0 if mask.dtype != bool else mask
        
        if effective_category == 'chamber':
            # Update chamber mask
            self._add_chamber_mask(binary_mask, mask_id, color, rebuild_viz)
        elif effective_category == 'hive':
            # Update hive mask
            self._add_hive_mask(binary_mask, mask_id, color, rebuild_viz)
        elif effective_category == 'pollen':
            # Update pollen mask
            self._add_pollen_mask(binary_mask, mask_id, color, rebuild_viz)
        else:
            # Default: bee annotation (multi-instance)
            self._add_bee_mask(binary_mask, mask_id, color, rebuild_viz)
    
    def _add_bee_mask(self, binary_mask, mask_id=None, color=None, rebuild_viz=True, category='bee'):
        """Add an instance mask to the appropriate mask array (bee, chamber, or hive)
        
        Args:
            binary_mask: Boolean array indicating instance pixels
            mask_id: Optional persistent ID for this instance
            color: Optional (r,g,b) color tuple
            rebuild_viz: If True, rebuild visualization immediately
            category: Instance category ('bee', 'chamber', 'hive', or 'pollen')
        """
        # Get reference to the appropriate mask based on category
        if category == 'chamber':
            mask_array_name = 'chamber_mask'
        elif category == 'hive':
            mask_array_name = 'hive_mask'
        elif category == 'pollen':
            mask_array_name = 'pollen_mask'
        else:
            mask_array_name = 'bee_mask'
        
        # Initialize mask if needed
        if getattr(self, mask_array_name) is None:
            h, w = binary_mask.shape[:2]
            setattr(self, mask_array_name, np.zeros((h, w), dtype=np.int32))
            self._cached_overlay = None  # Clear cache
        
        mask_array = getattr(self, mask_array_name)
        
        # Guard against stale callbacks (e.g. async SAM2) where the mask shape
        # no longer matches the re-initialised mask array (different video frame size).
        if binary_mask.shape[:2] != mask_array.shape[:2]:
            print(f"Warning: add_mask ignored – binary_mask shape {binary_mask.shape[:2]} "
                  f"doesn't match {mask_array_name} shape {mask_array.shape[:2]} "
                  f"(likely stale async result from previous video)")
            return
        
        # Assign or use provided mask ID
        if mask_id is None:
            mask_id = self.next_mask_id
            self.next_mask_id += 1
        else:
            # Update next_mask_id if necessary to avoid conflicts
            if mask_id >= self.next_mask_id:
                self.next_mask_id = mask_id + 1

        self.set_instance_visible(mask_id, category, True, rebuild=False)
        
        # Add to the appropriate mask array
        mask_array[binary_mask] = mask_id
        
        # Mark combined mask cache as dirty
        self._combined_mask_dirty = True
        
        # Invalidate cached instance IDs since we added a new one
        self._cached_instance_ids = None
        
        # Store color based on category. Non-bee categories always use fixed
        # category colors, even if a stale per-instance color was passed in.
        self.ensure_instance_color(mask_id, category, preferred_color=color)
        
        # Initialize metadata and store category
        if mask_id not in self.annotation_metadata:
            self.annotation_metadata[mask_id] = {}
        self.annotation_metadata[mask_id]['category'] = category
        
        # Rebuild visualization only if requested
        if rebuild_viz:
            self.rebuild_visualizations()
        self.selected_mask_idx = mask_id
        
        # Emit signal to notify that annotations have changed
        self.annotation_changed.emit()
    
    def _add_chamber_mask(self, binary_mask, mask_id=None, color=None, rebuild_viz=True):
        """Add chamber instance to combined mask with category='chamber'
        
        Args:
            binary_mask: Boolean array indicating chamber pixels
            mask_id: Optional persistent ID for this instance
            color: Optional (r,g,b) color tuple. If None, uses default red for chamber
            rebuild_viz: If True, rebuild visualization immediately
        """
        # Add as a regular instance with chamber category
        self._add_bee_mask(binary_mask, mask_id=mask_id, color=color, rebuild_viz=rebuild_viz, category='chamber')
    
    def _add_hive_mask(self, binary_mask, mask_id=None, color=None, rebuild_viz=True):
        """Add hive instance to combined mask with category='hive'
        
        Args:
            binary_mask: Boolean array indicating hive pixels
            mask_id: Optional persistent ID for this instance
            color: Optional (r,g,b) color tuple. If None, uses default yellow for hive
            rebuild_viz: If True, rebuild visualization immediately
        """
        # Add as a regular instance with hive category
        self._add_bee_mask(binary_mask, mask_id=mask_id, color=color, rebuild_viz=rebuild_viz, category='hive')
    
    def _add_pollen_mask(self, binary_mask, mask_id=None, color=None, rebuild_viz=True):
        """Add pollen instance to combined mask with category='pollen'
        
        Args:
            binary_mask: Boolean array indicating pollen pixels
            mask_id: Optional persistent ID for this instance
            color: Optional (r,g,b) color tuple. If None, uses default orange for pollen
            rebuild_viz: If True, rebuild visualization immediately
        """
        # Add as a regular instance with pollen category
        self._add_bee_mask(binary_mask, mask_id=mask_id, color=color, rebuild_viz=rebuild_viz, category='pollen')
    
    def _get_mask_array_for_instance(self, instance_id, category=None):
        """Get the appropriate mask array for a given instance ID
        
        Args:
            instance_id: Instance ID to look up
            category: Optional explicit category ('bee', 'chamber', 'hive', 'pollen')
            
        Returns:
            Tuple of (mask_array, category) or (None, None) if not found
        """
        if instance_id <= 0:
            return None, None
        
        # Use explicit category first. Otherwise prefer selection/editing state
        # before falling back to metadata; duplicate same-ID masks can otherwise
        # make the wrong category win.
        if category is None and instance_id == self.editing_instance_id:
            category = self.editing_instance_category
        if category is None and instance_id == self.selected_mask_idx:
            category = self.selected_instance_category
        if category is None:
            category = self.annotation_metadata.get(instance_id, {}).get('category')
        if category is None:
            for candidate_category, candidate_mask in [
                ('bee', self.bee_mask),
                ('chamber', self.chamber_mask),
                ('hive', self.hive_mask),
                ('pollen', self.pollen_mask)
            ]:
                if candidate_mask is not None and np.any(candidate_mask == instance_id):
                    category = candidate_category
                    break
        if category is None:
            category = 'bee'
        
        if category == 'chamber':
            return self.chamber_mask, 'chamber'
        elif category == 'hive':
            return self.hive_mask, 'hive'
        elif category == 'pollen':
            return self.pollen_mask, 'pollen'
        else:
            return self.bee_mask, 'bee'
    
    def _find_instance_at_point(self, x, y, category=None):
        """Find which instance (if any) is at the given point

        Args:
            x: X coordinate
            y: Y coordinate
            category: If given ('bee', 'chamber', 'hive', or 'pollen'), only search that
                      type's mask. Otherwise all masks are searched.

        Returns:
            Instance ID at that point, or 0 if no instance
        """
        instance_id, _ = self._find_instance_entry_at_point(x, y, category)
        return instance_id
    
    def _get_binary_mask_for_instance(self, instance_id):
        """Get binary mask for a specific instance ID
        
        Args:
            instance_id: Instance ID to extract
            
        Returns:
            Binary mask (H×W uint8) or None if instance not found
        """
        if instance_id <= 0:
            return None
        
        # Find which mask contains this instance
        mask_array, category = self._get_mask_array_for_instance(instance_id)
        if mask_array is not None:
            return (mask_array == instance_id).astype(np.uint8) * 255
        
        return None
    
    def change_instance_category(self, instance_id, new_category, old_category=None):
        """Move an instance from its current category to a new category
        
        Args:
            instance_id: Instance ID to move
            new_category: New category ('bee', 'chamber', 'hive', or 'pollen')
            old_category: Optional current category, used when the same numeric
                instance ID exists in multiple category masks.
        """
        if instance_id <= 0:
            return

        valid_categories = {'bee', 'chamber', 'hive', 'pollen'}
        if new_category not in valid_categories:
            print(f"Warning: Unknown category '{new_category}'")
            return

        categories = [
            ('bee', 'bee_mask'),
            ('chamber', 'chamber_mask'),
            ('hive', 'hive_mask'),
            ('pollen', 'pollen_mask')
        ]
        found_categories = []
        mask_shape = None

        editing_matches = (
            self.editing_instance_id == instance_id and
            self.editing_mask is not None and
            (
                old_category is None or
                self.editing_instance_category is None or
                self.editing_instance_category == old_category
            )
        )
        if editing_matches:
            mask_shape = self.editing_mask.shape
            if self.editing_instance_category:
                found_categories.append(self.editing_instance_category)

        for category, mask_attr in categories:
            mask_array = getattr(self, mask_attr)
            if mask_array is None:
                continue
            if mask_shape is None:
                mask_shape = mask_array.shape
            if mask_array.shape != mask_shape:
                continue
            if np.any(mask_array == instance_id):
                found_categories.append(category)

        if mask_shape is None and self.current_image is not None:
            mask_shape = self.current_image.shape[:2]

        source_pixels = (
            np.zeros(mask_shape, dtype=bool)
            if mask_shape is not None else None
        )
        if editing_matches and source_pixels is not None:
            source_pixels |= self.editing_mask > 0

        for _category, mask_attr in categories:
            mask_array = getattr(self, mask_attr)
            if mask_array is None or source_pixels is None:
                continue
            if mask_array.shape != source_pixels.shape:
                continue
            source_pixels |= mask_array == instance_id

        has_pixels = source_pixels is not None and np.any(source_pixels)
        if not has_pixels and instance_id not in self.annotation_metadata:
            print(f"Warning: Instance {instance_id} not found in any mask")
            return

        old_color = self.mask_colors.get(instance_id)
        old_metadata = dict(self.annotation_metadata.get(instance_id, {}))
        old_category_label = old_category or old_metadata.get('category')
        if not old_category_label and found_categories:
            old_category_label = ','.join(sorted(set(found_categories)))
        if not old_category_label:
            old_category_label = 'unknown'

        # Remove this numeric ID from every stored category mask first. The app
        # treats instance IDs as globally unique, so moving a category should also
        # clean up stale duplicate rows from previous failed moves.
        for _category, mask_attr in categories:
            mask_array = getattr(self, mask_attr)
            if mask_array is not None:
                mask_array[mask_array == instance_id] = 0

        if editing_matches:
            if source_pixels is not None:
                self.editing_mask = source_pixels.astype(np.uint8) * 255
            self.editing_instance_category = new_category
            self.selected_mask_idx = instance_id
            self.selected_instance_category = new_category
        elif has_pixels:
            # Add instance to the new mask array with the same ID.
            new_mask_array = self._get_mask_array_by_category(new_category)
            if new_mask_array is None:
                h, w = source_pixels.shape
                new_mask_array = np.zeros((h, w), dtype=np.int32)
                self._set_mask_array_by_category(new_category, new_mask_array)
            new_mask_array[source_pixels] = instance_id

        self.annotation_metadata[instance_id] = old_metadata
        self.annotation_metadata[instance_id]['category'] = new_category

        for category, _mask_attr in categories:
            self.hidden_instance_keys.discard(self._instance_key(instance_id, category))
        self.set_instance_visible(instance_id, new_category, True, rebuild=False)

        # Update color. Non-bee categories have fixed colors; bees keep their
        # prior bee color when available.
        previous_category = old_category or old_metadata.get('category')
        preferred_color = old_color if previous_category == 'bee' else None
        self.ensure_instance_color(
            instance_id,
            new_category,
            preferred_color=preferred_color
        )
        
        # Invalidate caches
        self._combined_mask_dirty = True
        self._cached_instance_ids = None
        self._cached_bboxes.clear()
        
        # Rebuild visualization
        self.rebuild_visualizations()
        if editing_matches:
            self._update_editing_visualization()
        self.update_instance_labels()
        self.instance_selected.emit(instance_id, new_category)
        
        # Emit signal that annotations changed
        self.annotation_changed.emit()
        
        print(f"Moved instance {instance_id} from {old_category_label} to {new_category}")
    
    def _get_mask_array_by_category(self, category):
        """Get the mask array for a specific category"""
        if category == 'bee':
            return self.bee_mask
        elif category == 'chamber':
            return self.chamber_mask
        elif category == 'hive':
            return self.hive_mask
        elif category == 'pollen':
            return self.pollen_mask
        return None
    
    def _set_mask_array_by_category(self, category, mask_array):
        """Set the mask array for a specific category"""
        if category == 'bee':
            self.bee_mask = mask_array
        elif category == 'chamber':
            self.chamber_mask = mask_array
        elif category == 'hive':
            self.hive_mask = mask_array
        elif category == 'pollen':
            self.pollen_mask = mask_array

    def _instance_key(self, instance_id, category=None):
        """Stable key for per-instance UI state."""
        if category is None:
            category = self.annotation_metadata.get(instance_id, {}).get('category')
        if category is None and instance_id == self.editing_instance_id:
            category = self.editing_instance_category
        if category is None and instance_id == self.selected_mask_idx:
            category = self.selected_instance_category
        if category is None:
            category = 'bee'
        return (str(category), int(instance_id))

    def _remove_instance_from_other_category_masks(self, instance_id, keep_category):
        """Ensure one instance ID is represented by only one category mask."""
        removed_categories = []
        for category, mask_attr in [
            ('bee', 'bee_mask'),
            ('chamber', 'chamber_mask'),
            ('hive', 'hive_mask'),
            ('pollen', 'pollen_mask')
        ]:
            if category == keep_category:
                continue

            mask_array = getattr(self, mask_attr)
            if mask_array is not None and np.any(mask_array == instance_id):
                mask_array[mask_array == instance_id] = 0
                removed_categories.append(category)

        if removed_categories:
            self._combined_mask_dirty = True
            self._cached_instance_ids = None
            self._cached_bboxes = {
                key: value for key, value in self._cached_bboxes.items()
                if key != instance_id and key != self._instance_key(instance_id)
            }
            print(
                f"Removed duplicate instance {instance_id} mask(s) from "
                f"{', '.join(removed_categories)}; keeping {keep_category}."
            )

    def _normalize_duplicate_instance_categories(self):
        """Remove stale duplicate same-ID masks from non-metadata categories."""
        categories = [
            ('bee', 'bee_mask'),
            ('chamber', 'chamber_mask'),
            ('hive', 'hive_mask'),
            ('pollen', 'pollen_mask')
        ]
        appearances = {}
        for category, mask_attr in categories:
            mask_array = getattr(self, mask_attr)
            if mask_array is None:
                continue
            instance_ids = np.unique(mask_array)
            instance_ids = instance_ids[instance_ids > 0]
            for instance_id in instance_ids:
                appearances.setdefault(int(instance_id), []).append(category)

        removed_messages = []
        for instance_id, present_categories in appearances.items():
            if len(present_categories) <= 1:
                continue

            keep_category = self.annotation_metadata.get(instance_id, {}).get('category')
            if keep_category not in present_categories:
                keep_category = present_categories[-1]
                self.annotation_metadata.setdefault(instance_id, {})['category'] = keep_category

            removed_categories = []
            for category, mask_attr in categories:
                if category == keep_category:
                    continue
                mask_array = getattr(self, mask_attr)
                if mask_array is not None and np.any(mask_array == instance_id):
                    mask_array[mask_array == instance_id] = 0
                    removed_categories.append(category)

            if removed_categories:
                removed_messages.append(
                    f"{instance_id}: removed {', '.join(removed_categories)}, kept {keep_category}"
                )

        if removed_messages:
            self._combined_mask_dirty = True
            self._cached_instance_ids = None
            self._cached_bboxes.clear()
            print(
                "Cleaned duplicate category masks for instance IDs: " +
                "; ".join(removed_messages)
            )
            return True
        return False

    def get_instance_entries(self):
        """Return category-aware instance entries for UI display."""
        entries = []
        seen = set()

        for category, mask_array in [
            ('bee', self.bee_mask),
            ('chamber', self.chamber_mask),
            ('hive', self.hive_mask),
            ('pollen', self.pollen_mask)
        ]:
            if mask_array is None:
                continue
            instance_ids = np.unique(mask_array)
            instance_ids = instance_ids[instance_ids > 0]
            for instance_id in instance_ids:
                key = self._instance_key(int(instance_id), category)
                if key not in seen:
                    seen.add(key)
                    entries.append({'id': int(instance_id), 'category': category})

        for instance_id, metadata in self.annotation_metadata.items():
            category = metadata.get('category', 'bee')
            key = self._instance_key(instance_id, category)
            if key not in seen:
                seen.add(key)
                entries.append({'id': int(instance_id), 'category': category})

        if self.editing_instance_id > 0 and self.editing_mask is not None:
            category = self.editing_instance_category or self.annotation_metadata.get(
                self.editing_instance_id, {}
            ).get('category', self.current_annotation_type)
            key = self._instance_key(self.editing_instance_id, category)
            if key not in seen:
                entries.append({'id': int(self.editing_instance_id), 'category': category})

        category_order = {'bee': 0, 'hive': 1, 'chamber': 2, 'pollen': 3}
        return sorted(entries, key=lambda entry: (entry['id'], category_order.get(entry['category'], 99)))
    
    @property
    def combined_mask(self):
        """Legacy compatibility property: provides a readonly combined view of all masks
        
        This dynamically generates a single mask with all instances for backward compatibility.
        Note: This is readonly - modifications won't affect the underlying separate masks.
        Use the appropriate mask (bee_mask, chamber_mask, hive_mask, pollen_mask) directly for modifications.
        
        Returns:
            H×W int32 array with all instance IDs, or None if no masks exist
        """
        # Return None if no masks exist
        if self.bee_mask is None and self.chamber_mask is None and self.hive_mask is None and self.pollen_mask is None:
            return None
        
        # Use cached version if available and not dirty
        if self._combined_mask_cache is not None and not self._combined_mask_dirty:
            return self._combined_mask_cache
        
        #Determine dimensions
        if self.bee_mask is not None:
            h, w = self.bee_mask.shape
        elif self.chamber_mask is not None:
            h, w = self.chamber_mask.shape
        elif self.hive_mask is not None:
            h, w = self.hive_mask.shape
        elif self.pollen_mask is not None:
            h, w = self.pollen_mask.shape
        else:
            return None
        
        # Create combined view (simple approach: bee mask as base since it's most common)
        combined = np.zeros((h, w), dtype=np.int32)
        
        # Layer masks: bee first (base layer), then chamber, then hive
        # Later layers overwrite earlier ones where they overlap
        if self.bee_mask is not None:
            combined[self.bee_mask > 0] = self.bee_mask[self.bee_mask > 0]
        if self.chamber_mask is not None:
            if self.chamber_mask.shape[:2] == (h, w):
                combined[self.chamber_mask > 0] = self.chamber_mask[self.chamber_mask > 0]
            else:
                print(f"Warning: combined_mask skipping chamber_mask "
                      f"(shape {self.chamber_mask.shape[:2]} != {(h, w)})")
        if self.hive_mask is not None:
            if self.hive_mask.shape[:2] == (h, w):
                combined[self.hive_mask > 0] = self.hive_mask[self.hive_mask > 0]
            else:
                print(f"Warning: combined_mask skipping hive_mask "
                      f"(shape {self.hive_mask.shape[:2]} != {(h, w)})")
        if self.pollen_mask is not None:
            if self.pollen_mask.shape[:2] == (h, w):
                combined[self.pollen_mask > 0] = self.pollen_mask[self.pollen_mask > 0]
            else:
                print(f"Warning: combined_mask skipping pollen_mask "
                      f"(shape {self.pollen_mask.shape[:2]} != {(h, w)})")
        
        # Cache for future reads
        self._combined_mask_cache = combined
        self._combined_mask_dirty = False
        
        return combined
    
    @combined_mask.setter
    def combined_mask(self, value):
        """Setter for backward compatibility - mark cache as dirty"""
        # Setting to None clears all masks
        if value is None:
            self.bee_mask = None
            self.chamber_mask = None
            self.hive_mask = None
            self.pollen_mask = None
            self._combined_mask_cache = None
            self._combined_mask_dirty = True
        else:
            # For other values, just mark cache as dirty
            # Direct assignment not supported - use set_annotations instead
            self._combined_mask_dirty = True
            
    def rebuild_visualizations(self):
        """Rebuild all visualizations (masks and/or bboxes) based on display flags"""
        # Clear existing visualization items
        for item in self.mask_items:
            try:
                if item.scene() == self.scene:
                    self.scene.removeItem(item)
            except (RuntimeError, AttributeError):
                # Item already deleted by Qt or invalid
                pass
        self.mask_items = []
        self.editing_bbox_item = None
        
        # Clear bbox items properly before clearing the map
        for mask_id, rect_item in list(self.bbox_items_map.items()):
            try:
                if rect_item.scene() == self.scene:
                    self.scene.removeItem(rect_item)
            except (RuntimeError, AttributeError):
                # Item already deleted by Qt or invalid
                pass
        self.bbox_items_map = {}
        
        # Check if any masks exist
        has_masks = (self.bee_mask is not None or 
                     self.chamber_mask is not None or 
                     self.hive_mask is not None or
                     self.pollen_mask is not None)
        
        if not has_masks:
            self._cached_overlay = None
            # Still draw bbox-only annotations if we have them
            if self.show_bboxes:
                self._draw_bboxes_from_metadata()
                self._draw_or_update_editing_bbox()
            self._sync_editing_mask_visibility()
            return
        
        # Collect all instance IDs from all masks
        all_instance_ids = []
        mask_lookup = {}  # Map instance_id -> (mask_type, mask_array)
        
        if self.bee_mask is not None and self.annotation_type_visibility.get('bee', True):
            bee_ids = np.unique(self.bee_mask)
            bee_ids = bee_ids[bee_ids > 0]
            for instance_id in bee_ids:
                if not self.is_instance_enabled(instance_id, 'bee'):
                    continue
                mask_lookup[instance_id] = ('bee', self.bee_mask)
        
        if self.chamber_mask is not None and self.annotation_type_visibility.get('chamber', True):
            chamber_ids = np.unique(self.chamber_mask)
            chamber_ids = chamber_ids[chamber_ids > 0]
            for instance_id in chamber_ids:
                if not self.is_instance_enabled(instance_id, 'chamber'):
                    continue
                mask_lookup[instance_id] = ('chamber', self.chamber_mask)
        
        if self.hive_mask is not None and self.annotation_type_visibility.get('hive', True):
            hive_ids = np.unique(self.hive_mask)
            hive_ids = hive_ids[hive_ids > 0]
            for instance_id in hive_ids:
                if not self.is_instance_enabled(instance_id, 'hive'):
                    continue
                mask_lookup[instance_id] = ('hive', self.hive_mask)
        
        if self.pollen_mask is not None and self.annotation_type_visibility.get('pollen', True):
            pollen_ids = np.unique(self.pollen_mask)
            pollen_ids = pollen_ids[pollen_ids > 0]
            for instance_id in pollen_ids:
                if not self.is_instance_enabled(instance_id, 'pollen'):
                    continue
                mask_lookup[instance_id] = ('pollen', self.pollen_mask)
        
        # Build rendering order: chamber and hive first (background), bees last (on top)
        chamber_ids_visible = [iid for iid, (cat, _) in mask_lookup.items() if cat == 'chamber']
        hive_ids_visible = [iid for iid, (cat, _) in mask_lookup.items() if cat == 'hive']
        pollen_ids_visible = [iid for iid, (cat, _) in mask_lookup.items() if cat == 'pollen']
        bee_ids_visible = [iid for iid, (cat, _) in mask_lookup.items() if cat == 'bee']
        all_instance_ids = chamber_ids_visible + hive_ids_visible + pollen_ids_visible + bee_ids_visible
        
        # Draw segmentation masks if enabled
        if self._segmentation_overlays_visible() and len(all_instance_ids) > 0:
            # Get dimensions from the first non-None mask
            if self.pollen_mask is not None:
                h, w = self.pollen_mask.shape
            elif self.bee_mask is not None:
                h, w = self.bee_mask.shape
            elif self.chamber_mask is not None:
                h, w = self.chamber_mask.shape
            else:
                h, w = self.hive_mask.shape
            
            # Create composite overlay for all instances
            overlay = np.zeros((h, w, 4), dtype=np.uint8)
            
            # Render each instance with category-based color
            for instance_id in all_instance_ids:
                # Get category and mask array
                category, mask_array = mask_lookup[instance_id]
                
                # Get metadata
                metadata = self.annotation_metadata.get(instance_id, {})
                
                # Get color for this instance. This intentionally rewrites
                # stale colors for fixed-color categories.
                color = self.ensure_instance_color(instance_id, category)
                
                # Apply color with appropriate opacity
                mask_pixels = mask_array == instance_id
                if mask_pixels.shape[:2] != (h, w):
                    print(f"Warning: rebuild_visualizations skipping instance {instance_id} – "
                          f"mask shape {mask_pixels.shape[:2]} != overlay shape {(h, w)}")
                    continue
                alpha = self.mask_opacity
                overlay[mask_pixels] = [color[0], color[1], color[2], alpha]
            
            # Cache the overlay for incremental updates
            self._cached_overlay = overlay
            
            # Convert to QPixmap and add to scene
            q_image = QImage(overlay.data, w, h, w * 4, QImage.Format.Format_RGBA8888)
            pixmap = QPixmap.fromImage(q_image.copy())  # copy to prevent memory leak
            
            mask_item = QGraphicsPixmapItem(pixmap)
            mask_item.setZValue(1)
            mask_item.setVisible(self._segmentation_overlays_visible())
            self.scene.addItem(mask_item)
            self.mask_items.append(mask_item)
            
            # Add marker for selected instance using the optimized method
            if self.selected_mask_idx > 0:
                self.update_selection_border()
            
            # Draw outlines for hive and chamber instances (helps distinguish overlapping regions)
            self._draw_instance_outlines(all_instance_ids, mask_lookup)
        else:
            self._cached_overlay = None
        
        # Draw bounding boxes if enabled
        if self.show_bboxes:
            self._draw_bboxes()
        
        # Update instance labels
        self.update_instance_labels()
        self.update_aruco_labels()
        self._sync_editing_mask_visibility()
    
    def _draw_bboxes(self):
        """Draw bounding boxes for all instances from masks and metadata"""
        from PyQt6.QtGui import QPen, QColor
        from PyQt6.QtCore import Qt
        
        # Draw bboxes for all instance types from their separate masks
        mask_sources = [
            ('bee', self.bee_mask),
            ('chamber', self.chamber_mask),
            ('hive', self.hive_mask),
            ('pollen', self.pollen_mask)
        ]
        
        for category, mask_array in mask_sources:
            if mask_array is None:
                continue
            if not self.annotation_type_visibility.get(category, True):
                continue
            
            instance_ids = np.unique(mask_array)
            instance_ids = instance_ids[instance_ids > 0]
            
            for instance_id in instance_ids:
                if not self.is_bbox_instance_visible(instance_id, category):
                    continue
                if instance_id in self.bbox_items_map:
                    continue  # Already drawn
                # Get bbox from mask
                mask = (mask_array == instance_id)
                bbox = self.get_mask_bbox(mask.astype(np.uint8) * 255)
                
                if bbox is not None:
                    x, y, w, h = bbox
                    color = self.mask_colors.get(instance_id, (255, 255, 255))
                    
                    rect_item = OutwardBBoxRectItem(x, y, w, h)
                    pen = QPen(QColor(color[0], color[1], color[2]))
                    pen.setWidth(4)
                    rect_item.setPen(pen)
                    rect_item.setZValue(100)
                    
                    self.scene.addItem(rect_item)
                    self.mask_items.append(rect_item)
                    self.bbox_items_map[instance_id] = rect_item
        
        # Also draw bbox-only annotations from metadata
        self._draw_bboxes_from_metadata()
        self._draw_or_update_editing_bbox()
    
    def _draw_bboxes_from_metadata(self):
        """Draw bounding boxes from bbox-only annotations in metadata"""
        from PyQt6.QtGui import QPen, QColor
        from PyQt6.QtCore import Qt
        
        # Draw bbox-only annotations (those not already in bbox_items_map)
        for instance_id, metadata in self.annotation_metadata.items():
            # Skip if already drawn from a mask
            if instance_id in self.bbox_items_map:
                continue
            
            # Skip if this instance is currently being edited
            if instance_id == self.editing_instance_id:
                continue
            
            # Use the instance's own category for visibility check
            category = metadata.get('category', 'bee')
            if not self.annotation_type_visibility.get(category, True):
                continue
            if not self.is_bbox_instance_visible(instance_id, category):
                continue
            
            # Only draw if it's a bbox-only annotation with a valid bbox
            if 'bbox' in metadata and metadata['bbox'] != [0, 0, 0, 0]:
                x, y, w, h = metadata['bbox']
                color = self.mask_colors.get(instance_id, (255, 255, 255))
                
                rect_item = OutwardBBoxRectItem(x, y, w, h)
                pen = QPen(QColor(color[0], color[1], color[2]))
                pen.setWidth(4)
                rect_item.setPen(pen)
                rect_item.setZValue(100)
                
                self.scene.addItem(rect_item)
                self.mask_items.append(rect_item)
                self.bbox_items_map[instance_id] = rect_item

    def _remove_editing_bbox_item(self):
        """Remove the active-edit bbox item without touching annotation data."""
        if not self.editing_bbox_item:
            return

        try:
            if self.editing_bbox_item.scene() == self.scene:
                self.scene.removeItem(self.editing_bbox_item)
        except (RuntimeError, AttributeError):
            pass

        self.mask_items = [
            item for item in self.mask_items
            if item is not self.editing_bbox_item
        ]
        for instance_id, item in list(self.bbox_items_map.items()):
            if item is self.editing_bbox_item:
                del self.bbox_items_map[instance_id]
        self.editing_bbox_item = None

    def _get_editing_bbox(self):
        """Return the bbox for the active edit mask or bbox-only metadata."""
        if self.editing_instance_id <= 0:
            return None

        if self.editing_mask is not None and np.any(self.editing_mask > 0):
            return self.get_mask_bbox(self.editing_mask)

        metadata = self.annotation_metadata.get(self.editing_instance_id, {})
        bbox = metadata.get('bbox')
        if metadata.get('bbox_only', False) and bbox and bbox != [0, 0, 0, 0]:
            return tuple(bbox)

        return None

    def _draw_or_update_editing_bbox(self):
        """Draw the active edit bbox above all regular boxes."""
        if self.editing_instance_id <= 0 or not self.show_bboxes:
            self._remove_editing_bbox_item()
            return

        category = self.editing_instance_category or self.annotation_metadata.get(
            self.editing_instance_id, {}
        ).get('category', self.current_annotation_type)
        if not self.is_annotation_instance_visible(self.editing_instance_id, category):
            self._remove_editing_bbox_item()
            return

        bbox = self._get_editing_bbox()
        if bbox is None:
            self._remove_editing_bbox_item()
            return

        x, y, w, h = bbox
        color = self.mask_colors.get(self.editing_instance_id, (255, 255, 255))
        if self.editing_bbox_item is None:
            self.editing_bbox_item = OutwardBBoxRectItem(x, y, w, h)
            self.scene.addItem(self.editing_bbox_item)
            self.mask_items.append(self.editing_bbox_item)
        else:
            self.editing_bbox_item.setRect(QRectF(x, y, w, h))

        pen = QPen(QColor(color[0], color[1], color[2]))
        pen.setWidth(6)
        self.editing_bbox_item.setPen(pen)
        self.editing_bbox_item.setZValue(140)
        self.editing_bbox_item.setVisible(True)
        self.bbox_items_map[self.editing_instance_id] = self.editing_bbox_item

    def _sync_editing_bbox_visibility(self):
        """Apply current bbox/class/instance visibility to the active edit bbox."""
        if not self.editing_bbox_item or self.editing_instance_id <= 0:
            return

        category = self.editing_instance_category or self.annotation_metadata.get(
            self.editing_instance_id, {}
        ).get('category', self.current_annotation_type)
        self.editing_bbox_item.setVisible(
            self.show_bboxes and
            self.is_annotation_instance_visible(self.editing_instance_id, category)
        )
    
    def _draw_instance_outlines(self, instance_ids, mask_lookup):
        """Draw contour outlines for hive, chamber, and pollen instances to better distinguish overlapping regions
        
        Args:
            instance_ids: List of instance IDs to potentially draw outlines for
            mask_lookup: Dict mapping instance_id -> (category, mask_array)
        """
        from PyQt6.QtWidgets import QGraphicsPathItem
        from PyQt6.QtGui import QPainterPath, QPen, QColor
        
        # Only draw outlines for hive, chamber, and pollen instances
        categories_to_outline = {'hive', 'chamber', 'pollen'}
        
        for instance_id in instance_ids:
            if instance_id not in mask_lookup:
                continue
            
            category, mask_array = mask_lookup[instance_id]
            
            # Only draw outlines for hive and chamber
            if category not in categories_to_outline:
                continue
            
            # Check if this category is visible
            if not self.annotation_type_visibility.get(category, True):
                continue
            
            # Extract binary mask for this instance
            binary_mask = (mask_array == instance_id).astype(np.uint8)
            
            # Find contours (both external and internal/holes)
            contours, hierarchy = cv2.findContours(
                binary_mask,
                cv2.RETR_CCOMP,  # Retrieve both external and internal contours
                cv2.CHAIN_APPROX_SIMPLE
            )
            
            if not contours:
                continue
            
            # Get instance color for outline
            color = self.mask_colors.get(instance_id, (255, 255, 255))
            
            # Create path for all contours (external and internal)
            path = QPainterPath()
            
            if hierarchy is not None:
                hierarchy = hierarchy[0]  # Reshape from (1, n, 4) to (n, 4)
                
                for i, contour in enumerate(contours):
                    if len(contour) < 3:  # Skip degenerate contours
                        continue
                    
                    # Start at first point
                    first_point = contour[0][0]
                    path.moveTo(float(first_point[0]), float(first_point[1]))
                    
                    # Draw lines to other points
                    for point in contour[1:]:
                        pt = point[0]
                        path.lineTo(float(pt[0]), float(pt[1]))
                    
                    # Close the path
                    path.closeSubpath()
            else:
                # No hierarchy, just draw all contours
                for contour in contours:
                    if len(contour) < 3:
                        continue
                    
                    first_point = contour[0][0]
                    path.moveTo(float(first_point[0]), float(first_point[1]))
                    
                    for point in contour[1:]:
                        pt = point[0]
                        path.lineTo(float(pt[0]), float(pt[1]))
                    
                    path.closeSubpath()
            
            # Create graphics item with contrasting outline
            # Use white outline with black shadow for good visibility
            outline_item = QGraphicsPathItem(path)
            
            # Use a brighter/contrasting color for the outline
            # White outline is most visible on colored backgrounds
            pen = QPen(QColor(255, 255, 255), 2)  # White, 2px thick
            outline_item.setPen(pen)
            outline_item.setZValue(5)  # Above masks (1) but below selection border (10)
            outline_item.setVisible(self._segmentation_overlays_visible())
            
            self.scene.addItem(outline_item)
            self.mask_items.append(outline_item)
    
    def _update_overlay_region(self, affected_pixels, instance_id):
        """Incrementally update only the affected region of the visualization
        
        Args:
            affected_pixels: Boolean mask of pixels that changed
            instance_id: ID of instance being drawn, or None to erase (set to background)
        """
        if self.combined_mask is None:
            return
        
        # Initialize cached overlay if needed
        if self._cached_overlay is None:
            self.rebuild_visualizations()
            return
        
        # Update only affected pixels in cached overlay
        if instance_id is not None:
            # Drawing: set pixels to instance color
            category = self.annotation_metadata.get(instance_id, {}).get('category', 'bee')
            color = self.ensure_instance_color(instance_id, category)
            alpha = self.mask_opacity
            self._cached_overlay[affected_pixels] = [color[0], color[1], color[2], alpha]
        else:
            # Erasing: set pixels to transparent or underlying instance color
            # Check what's actually in the combined mask at these positions
            erase_mask = affected_pixels & (self.combined_mask == 0)
            self._cached_overlay[erase_mask] = [0, 0, 0, 0]
            
            # Update any pixels that now show a different instance
            reveal_mask = affected_pixels & (self.combined_mask > 0)
            if np.any(reveal_mask):
                revealed_ids = np.unique(self.combined_mask[reveal_mask])
                for revealed_id in revealed_ids:
                    if revealed_id > 0:
                        category = self.annotation_metadata.get(int(revealed_id), {}).get('category', 'bee')
                        color = self.ensure_instance_color(int(revealed_id), category)
                        alpha = self.mask_opacity
                        reveal_pixels = reveal_mask & (self.combined_mask == revealed_id)
                        self._cached_overlay[reveal_pixels] = [color[0], color[1], color[2], alpha]
        
        # Update the pixmap from cached overlay
        h, w = self._cached_overlay.shape[:2]
        q_image = QImage(self._cached_overlay.data, w, h, w * 4, QImage.Format.Format_RGBA8888)
        pixmap = QPixmap.fromImage(q_image.copy())  # copy to prevent memory leak
        
        # Update existing mask item or create new one
        if self.mask_items:
            self.mask_items[0].setPixmap(pixmap)
        else:
            mask_item = QGraphicsPixmapItem(pixmap)
            mask_item.setZValue(1)
            mask_item.setVisible(self._segmentation_overlays_visible())
            self.scene.addItem(mask_item)
            self.mask_items.append(mask_item)
        
        # Update selection marker if we modified the selected instance
        if self.selected_mask_idx > 0 and (instance_id == self.selected_mask_idx or instance_id is None):
            # Refresh marker for selected instance
            selected_mask_binary = (self.combined_mask == self.selected_mask_idx).astype(np.uint8) * 255
            self.add_selection_border(selected_mask_binary)
    
    def add_mask_visualization(self, mask, color=None):
        """Add mask visualization overlay
        
        Args:
            mask: Binary mask array
            color: Optional (r,g,b) color tuple. If None, generates random color
        """
        # Create colored overlay
        h, w = mask.shape
        overlay = np.zeros((h, w, 4), dtype=np.uint8)
        
        # Use provided color or generate random one
        if color is None:
            color = np.random.randint(0, 255, 3)
        self.mask_colors.append(color)
        overlay[mask > 0] = [color[0], color[1], color[2], self.mask_opacity]
        
        # Convert to QPixmap (make copy to avoid memory leak)
        q_image = QImage(overlay.data, w, h, w * 4, QImage.Format.Format_RGBA8888)
        pixmap = QPixmap.fromImage(q_image.copy())
        
        # Add to scene
        mask_item = QGraphicsPixmapItem(pixmap)
        mask_item.setZValue(1)  # Above image
        mask_item.setVisible(self._segmentation_overlays_visible())
        self.scene.addItem(mask_item)
        self.mask_items.append(mask_item)
        
    def update_mask_visualization(self, idx):
        """DEPRECATED: Use rebuild_visualizations() instead"""
        # This method is no longer used with integer mask format
        # Just call rebuild_visualizations to update everything
        self.rebuild_visualizations()

    def _add_instance_label(self, instance_id, category, center_x, center_y, color=None):
        """Add a compact fixed-size label near an instance centroid."""
        if not self.is_annotation_instance_visible(instance_id, category):
            return

        if color is None:
            color = self.mask_colors.get(instance_id, (255, 255, 255))

        label_text = str(instance_id)
        text_item = QGraphicsTextItem()
        text_item.setHtml(
            f'<div style="background-color: rgba(0, 0, 0, 175); '
            f'color: rgb(255, 255, 255); '
            f'border: 2px solid rgb({color[0]}, {color[1]}, {color[2]}); '
            f'padding: 2px 5px; font-size: 14px; font-weight: bold;">'
            f'{label_text}</div>'
        )
        text_item.setFlag(QGraphicsItem.GraphicsItemFlag.ItemIgnoresTransformations, True)
        text_item.setZValue(155)

        text_rect = text_item.boundingRect()
        text_item.setPos(
            float(center_x) + 8.0,
            float(center_y) - text_rect.height() / 2.0 - 8.0
        )
        self.scene.addItem(text_item)
        self.label_items.append(text_item)

    def update_instance_labels(self):
        """Create or update text labels showing instance numbers on the canvas"""
        for label_item in self.label_items:
            try:
                if label_item.scene() == self.scene:
                    self.scene.removeItem(label_item)
            except (RuntimeError, AttributeError):
                pass
        self.label_items = []
        
        if not self.labels_visible:
            return

        labeled_keys = set()

        if self._segmentation_overlays_visible():
            for category, mask_array in [
                ('chamber', self.chamber_mask),
                ('hive', self.hive_mask),
                ('pollen', self.pollen_mask),
                ('bee', self.bee_mask),
            ]:
                if mask_array is None:
                    continue
                if not self.annotation_type_visibility.get(category, True):
                    continue

                instance_ids = np.unique(mask_array)
                instance_ids = instance_ids[instance_ids > 0]
                for instance_id in instance_ids:
                    instance_id = int(instance_id)
                    if not self.is_instance_enabled(instance_id, category):
                        continue

                    instance_mask = mask_array == instance_id
                    y_coords, x_coords = np.nonzero(instance_mask)
                    if len(y_coords) == 0:
                        continue

                    self._add_instance_label(
                        instance_id,
                        category,
                        float(np.mean(x_coords)),
                        float(np.mean(y_coords))
                    )
                    labeled_keys.add((instance_id, category))

            if self.editing_instance_id > 0 and self.editing_mask is not None:
                category = self.editing_instance_category or self.annotation_metadata.get(
                    self.editing_instance_id, {}
                ).get('category', self.current_annotation_type)
                if self._is_editing_mask_visible():
                    editing_pixels = self.editing_mask > 0
                    if np.any(editing_pixels):
                        y_coords, x_coords = np.nonzero(editing_pixels)
                        self._add_instance_label(
                            self.editing_instance_id,
                            category,
                            float(np.mean(x_coords)),
                            float(np.mean(y_coords))
                        )
                        labeled_keys.add((self.editing_instance_id, category))

        # If bboxes are visible, use bbox centers for bbox-only instances or for
        # cases where masks are hidden.
        if self.show_bboxes and self.bbox_items_map:
            to_remove = []

            for instance_id, rect_item in self.bbox_items_map.items():
                try:
                    category = self.annotation_metadata.get(instance_id, {}).get('category', 'bee')
                    if not self.annotation_type_visibility.get(category, True):
                        continue
                    if not self.is_instance_enabled(instance_id, category):
                        continue
                    if (instance_id, category) in labeled_keys:
                        continue

                    rect = rect_item.rect()
                    self._add_instance_label(
                        instance_id,
                        category,
                        rect.center().x(),
                        rect.center().y()
                    )
                    labeled_keys.add((instance_id, category))

                except (RuntimeError, AttributeError):
                    to_remove.append(instance_id)

            for instance_id in to_remove:
                if instance_id in self.bbox_items_map:
                    del self.bbox_items_map[instance_id]
    
    def set_labels_visible(self, visible):
        """Show or hide instance number labels
        
        Args:
            visible: True to show labels, False to hide
        """
        self.labels_visible = visible
        
        if visible:
            # Update and show labels
            self.update_instance_labels()
        else:
            # Hide all labels
            for label_item in self.label_items:
                try:
                    if label_item.scene() == self.scene:
                        self.scene.removeItem(label_item)
                except (RuntimeError, AttributeError):
                    # Item already deleted by Qt or invalid
                    pass
            self.label_items = []
    
    def toggle_labels(self):
        """Toggle visibility of instance number labels"""
        self.set_labels_visible(not self.labels_visible)

    def _clear_aruco_label_items(self):
        """Remove ArUco overlay labels from the scene."""
        for label_item in self.aruco_label_items:
            try:
                if label_item.scene() == self.scene:
                    self.scene.removeItem(label_item)
            except (RuntimeError, AttributeError):
                # Item already deleted by Qt or invalid
                pass
        self.aruco_label_items = []

    def set_aruco_labels_visible(self, visible):
        """Show or hide ArUco marker labels on the canvas."""
        self.aruco_labels_visible = visible
        if visible:
            self.update_aruco_labels()
        else:
            self._clear_aruco_label_items()

    def set_aruco_instance_labels(self, instance_to_aruco, rebuild=True):
        """Set video-level ArUco labels keyed by instance ID."""
        self.aruco_instance_labels = {
            int(instance_id): aruco_id
            for instance_id, aruco_id in (instance_to_aruco or {}).items()
        }
        if rebuild:
            self.update_aruco_labels()

    def update_aruco_labels(self):
        """Draw ArUco IDs next to matching bee instances."""
        self._clear_aruco_label_items()

        if not self.aruco_labels_visible or not self.aruco_instance_labels:
            return

        for instance_id, aruco_id in sorted(self.aruco_instance_labels.items()):
            metadata = self.annotation_metadata.get(instance_id, {})
            if metadata.get('category', 'bee') != 'bee':
                continue
            if not self.annotation_type_visibility.get('bee', True):
                continue
            if not self.is_instance_enabled(instance_id, 'bee'):
                continue

            bbox = None
            if instance_id in self.bbox_items_map:
                rect = self.bbox_items_map[instance_id].rect()
                bbox = (rect.x(), rect.y(), rect.width(), rect.height())
            else:
                bbox = self.get_instance_bbox_cached(instance_id)

            if not bbox:
                continue

            x, y, _w, _h = bbox
            label_text = f"ArUco {aruco_id}"
            text_item = QGraphicsTextItem()
            text_item.setHtml(
                '<div style="'
                'background-color: rgba(0, 0, 0, 190); '
                'color: rgb(0, 255, 255); '
                'font-size: 24px; '
                'font-weight: bold; '
                'padding: 2px 4px;'
                f'">{label_text}</div>'
            )
            text_item.setPos(float(x) + 5, max(0.0, float(y) - 34))
            text_item.setZValue(120)
            self.scene.addItem(text_item)
            self.aruco_label_items.append(text_item)

    def is_instance_enabled(self, instance_id, category=None):
        """Return whether an individual instance switch is enabled."""
        return self._instance_key(instance_id, category) not in self.hidden_instance_keys

    def is_annotation_instance_visible(self, instance_id, category=None):
        """Return effective visibility: class switch first, then instance switch."""
        if category is None:
            category = self.annotation_metadata.get(instance_id, {}).get('category', 'bee')
        return (
            self.annotation_type_visibility.get(category, True) and
            self.is_instance_enabled(instance_id, category)
        )

    def set_instance_visible(self, instance_id, category, visible, rebuild=True):
        """Show or hide one instance without changing annotation data."""
        key = self._instance_key(instance_id, category)
        if visible:
            self.hidden_instance_keys.discard(key)
        else:
            self.hidden_instance_keys.add(key)
            if self.selected_bbox_id == instance_id:
                self._deselect_bbox()

        if rebuild:
            self.rebuild_visualizations()

    def toggle_selected_instance_visibility(self):
        """Toggle visibility for the currently selected instance."""
        if self.selected_mask_idx <= 0:
            return False

        category = (
            self.selected_instance_category or
            self.annotation_metadata.get(self.selected_mask_idx, {}).get('category', self.current_annotation_type)
        )

        if not self.annotation_type_visibility.get(category, True):
            self.instance_visibility_toggle_blocked.emit(self.selected_mask_idx, category)
            return False

        visible = not self.is_instance_enabled(self.selected_mask_idx, category)
        self.set_instance_visible(self.selected_mask_idx, category, visible)
        self.instance_visibility_changed.emit(self.selected_mask_idx, category, visible)
        return True

    def is_bbox_instance_visible(self, instance_id, category=None):
        """Backward-compatible alias for the unified instance visibility switch."""
        return self.is_instance_enabled(instance_id, category)

    def set_bbox_instance_visible(self, instance_id, visible, rebuild=True, category=None):
        """Backward-compatible alias for the unified instance visibility switch."""
        self.set_instance_visible(instance_id, category, visible, rebuild=rebuild)
            
    def set_annotations(self, annotations, mask_colors=None):
        """Load annotations for current frame
        
        Args:
            annotations: List of annotation dictionaries (all types: bee, chamber, hive)
            mask_colors: Optional dict mapping mask_id to (r,g,b) color tuple
        """
        # Commit any pending edits first
        if self.editing_instance_id > 0:
            self.commit_editing()
        
        # Clear existing masks and free memory
        for mask_attr in ['bee_mask', 'chamber_mask', 'hive_mask', 'pollen_mask']:
            if getattr(self, mask_attr) is not None:
                delattr(self, mask_attr)
                setattr(self, mask_attr, None)
        
        # Clear cached overlay
        if self._cached_overlay is not None:
            del self._cached_overlay
            self._cached_overlay = None
        
        for item in self.mask_items:
            try:
                if item.scene() == self.scene:
                    self.scene.removeItem(item)
            except (RuntimeError, AttributeError):
                # Item already deleted by Qt or invalid
                pass
            # Explicitly delete the item to free Qt resources
            try:
                item.setPixmap(None)  # Release pixmap data
            except:
                pass
            del item
        
        # Clear mask references
        self.mask_items = []
        self.mask_colors = {}  # Reset to dict
        self.next_mask_id = 1
        self.annotation_metadata = {}  # Clear metadata
        
        # Clear selection border
        if self.selection_border_item:
            try:
                if self.selection_border_item.scene() == self.scene:
                    self.scene.removeItem(self.selection_border_item)
            except (RuntimeError, AttributeError):
                # Item already deleted by Qt or invalid
                pass
            try:
                self.selection_border_item.setPixmap(None)
            except:
                pass
            del self.selection_border_item
            self.selection_border_item = None
        
        # Reset selection
        self.selected_mask_idx = -1
        self.selected_instance_category = None
        self.active_sam2_mask_idx = -1
        self.editing_instance_category = None
        self.hidden_instance_keys.clear()
        self.hidden_bbox_instance_ids.clear()
        self._clear_aruco_label_items()
        self._clear_brush_history()
        
        # Clear cached data for performance optimization
        self._cached_instance_ids = None
        self._cached_bboxes = {}
        
        # Force garbage collection to free memory
        import gc
        gc.collect()
        
        if not annotations:
            return
        
        # Determine dimensions from current image or first mask
        has_masks = any('mask' in ann for ann in annotations)
        h, w = None, None
        
        if self.current_image is not None:
            h, w = self.current_image.shape[:2]
        elif has_masks:
            first_mask_ann = next((ann for ann in annotations if 'mask' in ann), None)
            if first_mask_ann:
                h, w = first_mask_ann['mask'].shape[:2]
        
        if h is None or w is None:
            return  # Can't determine size
        
        # Initialize separate masks for each category
        self.bee_mask = np.zeros((h, w), dtype=np.int32)
        self.chamber_mask = np.zeros((h, w), dtype=np.int32)
        self.hive_mask = np.zeros((h, w), dtype=np.int32)
        self.pollen_mask = np.zeros((h, w), dtype=np.int32)
        
        # Build separate masks from annotations
        for ann in annotations:
            # Get instance ID
            mask_id = ann.get('mask_id', self.next_mask_id)
            if mask_id >= self.next_mask_id:
                self.next_mask_id = mask_id + 1
            
            # Get category (defaults to 'bee' for backward compatibility)
            category = ann.get('category', 'bee')
            
            # Determine which mask array to use
            if category == 'chamber':
                mask_array = self.chamber_mask
            elif category == 'hive':
                mask_array = self.hive_mask
            elif category == 'pollen':
                mask_array = self.pollen_mask
            else:  # bee
                mask_array = self.bee_mask

            # Store or assign color. Fixed-color categories ignore stale saved
            # per-instance colors; bees can keep their saved color.
            preferred_color = mask_colors.get(mask_id) if mask_colors else None
            self.ensure_instance_color(mask_id, category, preferred_color=preferred_color)
            
            if 'mask' in ann:
                # Annotation with mask
                mask = ann['mask']
                if not isinstance(mask, np.ndarray):
                    continue
                
                # Validate mask dimensions
                if mask.shape[:2] != (h, w):
                    print(f"Warning: Skipping instance {mask_id} - mask dimensions {mask.shape[:2]} don't match image dimensions {(h, w)}")
                    # Still store metadata for bbox-only fallback
                    metadata = {k: v for k, v in ann.items() if k != 'mask'}
                    metadata['bbox_only'] = True
                    metadata['category'] = category
                    self.annotation_metadata[mask_id] = metadata
                    continue
                
                # Store metadata (everything except mask)
                metadata = {k: v for k, v in ann.items() if k != 'mask'}
                metadata['category'] = category
                self.annotation_metadata[mask_id] = metadata
                
                # Add to the appropriate mask array
                binary_mask = mask > 0 if mask.dtype != bool else mask
                mask_array[binary_mask] = mask_id
            else:
                # Bbox-only annotation - store metadata for later drawing
                metadata = ann.copy()
                metadata['category'] = category
                self.annotation_metadata[mask_id] = metadata
        
        # Mark combined mask cache as dirty since we modified masks
        self._combined_mask_dirty = True
        cleaned_duplicates = self._normalize_duplicate_instance_categories()
        
        # Rebuild visualizations once at the end
        self.rebuild_visualizations()
        if cleaned_duplicates:
            self.annotation_changed.emit()
    
    def _set_bbox_annotations(self, bbox_annotations, mask_colors=None):
        """DEPRECATED: Bbox-only annotations are now handled in main set_annotations flow
        
        This method is kept for backward compatibility but should not be called
        since bbox-only annotations now go through the unified annotation path.
        """
        # This method is deprecated - bbox annotations are handled in set_annotations
        pass
    
    def get_annotations(self):
        """Get all current annotations (bee, chamber, hive, and pollen)
        
        Includes the editing mask if currently editing.
        Also includes bbox-only annotations if present.
        Preserves metadata including category.
        """
        # Flush any pending visualization updates from throttled drawing
        self._flush_pending_viz_update()
        
        annotations = []
        
        # Process all four mask types
        mask_sources = [
            ('bee', self.bee_mask),
            ('chamber', self.chamber_mask),
            ('hive', self.hive_mask),
            ('pollen', self.pollen_mask)
        ]
        
        for category, mask_array in mask_sources:
            if mask_array is None:
                continue
            
            # Get unique instance IDs from this mask (excluding background 0)
            instance_ids = np.unique(mask_array)
            instance_ids = instance_ids[instance_ids > 0].tolist()
            
            # Add editing instance ID if currently editing and matches this category
            if (self.editing_instance_id > 0 and 
                self.editing_mask is not None and 
                self.annotation_metadata.get(self.editing_instance_id, {}).get('category', 'bee') == category):
                if self.editing_instance_id not in instance_ids:
                    instance_ids.append(self.editing_instance_id)
            
            for instance_id in instance_ids:
                # Check if this is the editing instance
                if instance_id == self.editing_instance_id and self.editing_mask is not None:
                    # Use editing mask
                    binary_mask = self.editing_mask.copy()
                else:
                    # Extract binary mask from the appropriate mask array
                    binary_mask = (mask_array == instance_id).astype(np.uint8) * 255
                
                # Start with stored metadata if available
                if instance_id in self.annotation_metadata:
                    ann = self.annotation_metadata[instance_id].copy()
                else:
                    ann = {}
                
                # Always update/add these core fields
                ann['mask'] = binary_mask
                ann['label'] = ann.get('label', f'instance_{instance_id}')
                ann['area'] = int(np.sum(binary_mask > 0))
                ann['mask_id'] = int(instance_id)
                ann['category'] = category
                
                # Auto-compute bbox from mask
                bbox = self.get_mask_bbox(binary_mask)
                if bbox is not None:
                    ann['bbox'] = list(bbox)  # [x, y, width, height]
                
                annotations.append(ann)
        
        # Also return bbox-only annotations (stored in metadata but not in combined_mask)
        # These don't have masks, just bboxes
        # Track which IDs were already added from combined_mask to avoid duplicates
        added_ids = {ann['mask_id'] for ann in annotations}
        
        for instance_id, metadata in self.annotation_metadata.items():
            if metadata.get('bbox_only', False) and instance_id not in added_ids:
                # Skip placeholder bboxes (0,0,0,0)
                if 'bbox' not in metadata or metadata['bbox'] == [0, 0, 0, 0]:
                    continue
                
                # This is a bbox-only annotation that wasn't already added
                ann = metadata.copy()
                ann['mask_id'] = int(instance_id)
                # Make sure bbox_only flag is set
                ann['bbox_only'] = True
                annotations.append(ann)
        
        return annotations
    
    def get_instance_ids(self):
        """
        Get list of all instance IDs (cached for performance)
        
        Returns:
            Sorted list of instance IDs
        """
        # Return cached list if available
        if self._cached_instance_ids is not None:
            return self._cached_instance_ids
        
        # Build instance ID list
        instance_ids_set = set()
        
        # 1. From all three mask arrays (instances with segmentations)
        for mask_array in [self.bee_mask, self.chamber_mask, self.hive_mask, self.pollen_mask]:
            if mask_array is not None:
                mask_instance_ids = np.unique(mask_array)
                mask_instance_ids = mask_instance_ids[mask_instance_ids > 0]  # Exclude background (0)
                instance_ids_set.update(mask_instance_ids.tolist())
        
        # 2. From annotation_metadata (bbox-only instances)
        instance_ids_set.update(self.annotation_metadata.keys())
        
        # 3. Add editing instance if currently in editing mode
        if self.editing_instance_id > 0 and self.editing_mask is not None:
            instance_ids_set.add(self.editing_instance_id)
        
        # Cache and return sorted list
        self._cached_instance_ids = sorted(list(instance_ids_set))
        return self._cached_instance_ids
    
    def reassign_instance_id(self, old_id, new_id):
        """Reassign an instance from old_id to new_id (for ArUco tracking)
        
        Args:
            old_id: Current instance ID
            new_id: New instance ID to assign
            
        Returns:
            bool: True if successful, False if failed
        """
        if old_id == new_id:
            return True
        
        # Find which mask array contains this instance
        mask_array, category = self._get_mask_array_for_instance(old_id)
        
        if mask_array is None:
            # Try metadata-only (bbox-only annotation)
            if old_id not in self.annotation_metadata:
                return False
            mask_array = None  # Will handle metadata-only below
        
        # Check if new_id already exists
        existing_ids = self.get_instance_ids()
        if new_id in existing_ids and new_id != old_id:
            # Merge instances: new_id exists, so we're merging old_id into it
            print(f"  ArUco reassignment: Merging instance {old_id} into existing instance {new_id}")
            
            if mask_array is not None:
                # Get target mask array for new_id
                target_mask_array, target_category = self._get_mask_array_for_instance(new_id)
                
                if target_mask_array is not None and target_category == category:
                    # Merge masks: pixels from old_id become new_id
                    target_mask_array[mask_array == old_id] = new_id
                    # Clear old_id from original mask
                    mask_array[mask_array == old_id] = 0
                else:
                    print(f"  Warning: Cannot merge - target instance {new_id} not found or category mismatch")
                    return False
            
            # Merge metadata (prefer existing new_id metadata, but add marker info from old_id if present)
            if old_id in self.annotation_metadata:
                old_metadata = self.annotation_metadata.pop(old_id)
                # Only transfer marker info if new_id doesn't have it
                if new_id in self.annotation_metadata:
                    if 'marker' in old_metadata and 'marker' not in self.annotation_metadata[new_id]:
                        self.annotation_metadata[new_id]['marker'] = old_metadata['marker']
            
            # Keep color of new_id
            if old_id in self.mask_colors:
                self.mask_colors.pop(old_id)
                
        else:
            # Simple reassignment: old_id becomes new_id
            if mask_array is not None:
                # Update mask array
                mask_array[mask_array == old_id] = new_id
            
            # Update metadata
            if old_id in self.annotation_metadata:
                metadata = self.annotation_metadata.pop(old_id)
                self.annotation_metadata[new_id] = metadata
            
            # Update colors
            if old_id in self.mask_colors:
                color = self.mask_colors.pop(old_id)
                category = self.annotation_metadata.get(new_id, {}).get('category', 'bee')
                self.ensure_instance_color(new_id, category, preferred_color=color)
            
            # Update next_mask_id if needed
            if new_id >= self.next_mask_id:
                self.next_mask_id = new_id + 1
        
        # Clear caches
        self._cached_instance_ids = None
        self._cached_bboxes = {}
        if self._cached_overlay is not None:
            del self._cached_overlay
            self._cached_overlay = None
        
        # Update selection if needed
        if self.selected_mask_idx == old_id:
            self.selected_mask_idx = new_id
        if self.editing_instance_id == old_id:
            self.editing_instance_id = new_id
        
        return True
    
    def get_instance_bbox_cached(self, instance_id, category=None):
        """
        Get bounding box for an instance (cached for performance)
        
        Args:
            instance_id: Instance ID
            
        Returns:
            tuple: (x, y, width, height) or None if mask is empty/invalid
        """
        # Check cache first
        cache_key = self._instance_key(instance_id, category)
        if cache_key in self._cached_bboxes:
            return self._cached_bboxes[cache_key]
        
        bbox = None
        
        # Try to get bbox from editing mask
        if instance_id == self.editing_instance_id and self.editing_mask is not None:
            bbox = self.get_mask_bbox(self.editing_mask)
        # Try to get bbox from combined_mask
        elif instance_id > 0:
            mask_array, resolved_category = self._get_mask_array_for_instance(instance_id, category)
            instance_mask = mask_array == instance_id if mask_array is not None else None
            if instance_mask is not None and np.any(instance_mask):
                bbox = self.get_mask_bbox(instance_mask)
        
        # Try to get bbox from metadata (for bbox-only annotations)
        if bbox is None and instance_id in self.annotation_metadata:
            metadata = self.annotation_metadata[instance_id]
            if 'bbox' in metadata and metadata['bbox'] != [0, 0, 0, 0]:
                bbox = tuple(metadata['bbox'])
        
        # Cache the result (even if None)
        if bbox is not None:
            self._cached_bboxes[cache_key] = bbox
        
        return bbox
    
    def zoom_to_instance_fast(self, idx, padding=50, category=None):
        """Fast version of zoom_to_instance using cached bounding box
        
        Args:
            idx: Instance ID (mask_id) to zoom to
            padding: Extra space around the instance (in pixels)
        """
        bbox = self.get_instance_bbox_cached(idx, category)
        if bbox:
            x, y, width, height = bbox
            self.zoom_to_rect(x, y, width, height, padding)
        
    def set_selected_instance(self, idx, category=None, zoom=True):
        """Set selected instance by ID.

        Args:
            idx: Instance ID.
            category: Optional explicit category for duplicate numeric IDs.
            zoom: Whether to recenter/zoom the view on the selected instance.
        """
        self.selected_mask_idx = idx
        self.selected_instance_category = category or self.annotation_metadata.get(idx, {}).get(
            'category', self.current_annotation_type
        )
        
        # If brush/eraser tool is active and instance has a segmentation, start editing mode
        # For bbox-only instances, wait until user actually starts drawing
        if self.current_tool in ['brush', 'eraser']:
            # Check if instance has segmentation data
            mask_array, _ = self._get_mask_array_for_instance(idx, self.selected_instance_category)
            has_segmentation = (
                mask_array is not None and
                np.any(mask_array == idx)
            )
            if has_segmentation:
                if zoom:
                    self.zoom_to_instance_fast(idx, category=self.selected_instance_category)
                
                # Now start editing
                self.start_editing_instance(idx, category=self.selected_instance_category)
                return
            # For bbox-only, just select and wait for user to start drawing
        
        # Try to zoom to the instance using cached bbox (much faster)
        # If bbox graphics item exists (bboxes are visible), use it
        if idx in self.bbox_items_map:
            self._select_bbox(idx, zoom=zoom)
        elif zoom:
            # Use cached bbox for fast zooming
            self.zoom_to_instance_fast(idx, category=self.selected_instance_category)
    
    def get_mask_bbox(self, mask):
        """
        Get bounding box of a mask
        
        Args:
            mask: numpy array representing the mask
            
        Returns:
            tuple: (x, y, width, height) or None if mask is empty
        """
        if mask is None or not np.any(mask):
            return None
        
        # Find coordinates where mask is non-zero
        coords = np.argwhere(mask > 0)
        if len(coords) == 0:
            return None
        
        # Get min/max coordinates (note: coords are in [row, col] format)
        y_min, x_min = coords.min(axis=0)
        y_max, x_max = coords.max(axis=0)
        
        width = x_max - x_min + 1
        height = y_max - y_min + 1
        
        return (int(x_min), int(y_min), int(width), int(height))
    
    def zoom_to_rect(self, x, y, width, height, padding=50):
        """
        Zoom and center view on a specific rectangle
        
        Args:
            x, y: Top-left corner of rectangle
            width, height: Size of rectangle
            padding: Extra space around the rectangle (in pixels)
        """
        if not self.image_item:
            return
        
        # Add padding
        x_padded = max(0, x - padding)
        y_padded = max(0, y - padding)
        width_padded = width + 2 * padding
        height_padded = height + 2 * padding
        
        # Create QRectF for the region
        rect = QRectF(x_padded, y_padded, width_padded, height_padded)
        
        # Fit the view to this rectangle
        self.fitInView(rect, Qt.AspectRatioMode.KeepAspectRatio)
        
        # Update zoom factor based on the transform
        transform = self.transform()
        self.zoom_factor = transform.m11()  # Get scale factor from transform matrix
    
    def zoom_to_instance(self, idx, padding=50):
        """Zoom to a specific instance by its mask_id
        
        Args:
            idx: Instance ID (mask_id) to zoom to
            padding: Extra space around the instance (in pixels)
        """
        if self.combined_mask is None or idx <= 0:
            return
        
        # Extract binary mask for this instance
        binary_mask = (self.combined_mask == idx).astype(np.uint8) * 255
        if not np.any(binary_mask > 0):
            return
        
        bbox = self.get_mask_bbox(binary_mask)
        if bbox is None:
            return
        
        x, y, width, height = bbox
        self.zoom_to_rect(x, y, width, height, padding)
    
    def highlight_instance(self, idx, category=None):
        """Highlight a specific instance by ID"""
        # Commit any edits to previous instance before switching
        target_category = category or self.annotation_metadata.get(idx, {}).get(
            'category', self.current_annotation_type
        )
        if (self.editing_instance_id > 0 and
            (self.editing_instance_id != idx or self.editing_instance_category != target_category)):
            self.commit_editing()

        # In bbox view mode, don't rebuild visualizations (would clear bbox selection)
        if self.show_bboxes:
            self.selected_mask_idx = idx
            self.selected_instance_category = target_category
            return
        
        self.selected_mask_idx = idx
        self.selected_instance_category = target_category
        
        # PERFORMANCE OPTIMIZATION: Only update the selection marker instead of rebuilding everything
        # This is much faster when there are many instances
        self.update_selection_border()
        self.instance_selected.emit(idx, target_category)
            
    def refresh_all_visualizations(self):
        """Refresh all mask visualizations with current selection highlighting"""
        self.rebuild_visualizations()
            
    def update_selection_border(self):
        """Update the selection marker for the currently selected instance
        
        This is a lightweight method that only updates the marker without rebuilding
        all visualizations. Much faster for instance switching.
        """
        # Remove old marker
        if self.selection_border_item:
            try:
                if self.selection_border_item.scene() == self.scene:
                    self.scene.removeItem(self.selection_border_item)
            except (RuntimeError, AttributeError):
                pass
            self.selection_border_item = None

        if not self._segmentation_overlays_visible():
            return
        
        # Add new marker if we have a valid selection
        if self.selected_mask_idx > 0:
            # Check if the selected instance's category is visible
            category = (
                self.selected_instance_category or
                self.annotation_metadata.get(self.selected_mask_idx, {}).get('category', 'bee')
            )
            if not self.is_annotation_instance_visible(self.selected_mask_idx, category):
                # Category is hidden, don't show selection marker
                return
            # Check if instance exists in combined_mask or is being edited
            if self.selected_mask_idx == self.editing_instance_id and self.editing_mask is not None:
                # Use editing mask
                binary_mask = self.editing_mask
            else:
                mask_array, _ = self._get_mask_array_for_instance(self.selected_mask_idx, category)
                if mask_array is None or not np.any(mask_array == self.selected_mask_idx):
                    return
                # Use combined mask
                binary_mask = (mask_array == self.selected_mask_idx).astype(np.uint8) * 255
            
            self.add_selection_border(binary_mask)
    
    def add_selection_border(self, mask):
        """Add a centroid marker for the selected mask."""
        # Remove previous marker if it exists
        if self.selection_border_item:
            try:
                if self.selection_border_item.scene() == self.scene:
                    self.scene.removeItem(self.selection_border_item)
            except (RuntimeError, AttributeError):
                # Item already deleted by Qt or invalid
                pass
            self.selection_border_item = None

        if not self._segmentation_overlays_visible():
            return
        
        # Check if the selected instance's category is visible
        if self.selected_mask_idx > 0:
            category = (
                self.selected_instance_category or
                self.annotation_metadata.get(self.selected_mask_idx, {}).get('category', 'bee')
            )
            if not self.is_annotation_instance_visible(self.selected_mask_idx, category):
                # Category is hidden, don't show selection marker
                return
        
        mask_pixels = mask > 0
        if not np.any(mask_pixels):
            return

        y_coords, x_coords = np.nonzero(mask_pixels)
        centroid_x = float(np.mean(x_coords))
        centroid_y = float(np.mean(y_coords))

        radius = 6.0
        marker_item = QGraphicsEllipseItem(-radius, -radius, radius * 2, radius * 2)
        marker_item.setPos(centroid_x, centroid_y)
        marker_item.setBrush(QBrush(QColor(255, 255, 0, 230)))

        pen = QPen(QColor(0, 0, 0))
        pen.setWidthF(1.5)
        pen.setCosmetic(True)
        marker_item.setPen(pen)

        marker_item.setZValue(150)  # Above masks and bbox lines
        marker_item.setFlag(QGraphicsItem.GraphicsItemFlag.ItemIgnoresTransformations, True)
        self.scene.addItem(marker_item)
        self.selection_border_item = marker_item
        
    def start_editing_instance(self, instance_id, category=None):
        """Start editing an instance in isolation mode
        
        Extracts the instance to a temporary editing mask so edits don't affect other instances
        until committed.
        
        Args:
            instance_id: ID of instance to edit
            category: Optional explicit category to edit
        """
        category = category or self.selected_instance_category or self.annotation_metadata.get(
            instance_id, {}
        ).get('category', self.current_annotation_type)

        if (self.editing_instance_id == instance_id and
            self.editing_instance_category == category and
            self.editing_mask is not None):
            return

        # Commit any previous edits
        if (self.editing_instance_id > 0 and
            (self.editing_instance_id != instance_id or self.editing_instance_category != category)):
            self.commit_editing()
        self._clear_brush_history()
        
        # Get the appropriate mask array for this instance
        mask_array, category = self._get_mask_array_for_instance(instance_id, category)
        
        # Ensure mask exists
        if mask_array is None:
            if self.current_image is not None:
                h, w = self.current_image.shape[:2]
                # Create the appropriate mask based on category
                if category == 'chamber' or self.current_annotation_type == 'chamber':
                    if self.chamber_mask is None:
                        self.chamber_mask = np.zeros((h, w), dtype=np.int32)
                    mask_array = self.chamber_mask
                elif category == 'hive' or self.current_annotation_type == 'hive':
                    if self.hive_mask is None:
                        self.hive_mask = np.zeros((h, w), dtype=np.int32)
                    mask_array = self.hive_mask
                elif category == 'pollen' or self.current_annotation_type == 'pollen':
                    if self.pollen_mask is None:
                        self.pollen_mask = np.zeros((h, w), dtype=np.int32)
                    mask_array = self.pollen_mask
                else:  # bee
                    if self.bee_mask is None:
                        self.bee_mask = np.zeros((h, w), dtype=np.int32)
                    mask_array = self.bee_mask
            else:
                return
        
        h, w = mask_array.shape
        self.editing_mask = np.zeros((h, w), dtype=np.uint8)
        self._remove_instance_from_other_category_masks(instance_id, category)
        
        # Check if this instance exists in mask_array or is bbox-only
        self._editing_started_with_pixels = bool(np.any(mask_array == instance_id))
        if self._editing_started_with_pixels:
            # Instance has segmentation - extract it
            instance_pixels = mask_array == instance_id
            self.editing_mask[instance_pixels] = 255
            mask_array[instance_pixels] = 0
            # Mark combined mask cache as dirty
            self._combined_mask_dirty = True
        else:
            # Instance is bbox-only or new - start with empty editing mask
            # User will draw segmentation with brush or SAM2
            pass
        
        self.editing_instance_id = instance_id
        self.editing_instance_category = category
        self.selected_mask_idx = instance_id  # Sync selected_mask_idx with editing_instance_id
        self.selected_instance_category = category
        self.set_instance_visible(instance_id, category, True, rebuild=False)
        
        # Stamp the correct category into annotation_metadata for new instances.
        # _get_mask_array_for_instance defaults to 'bee' when the instance is brand-new
        # (not yet in metadata), so we must record the actual current type here so
        # that get_annotations() and commit_editing() both resolve the right category.
        if instance_id not in self.annotation_metadata:
            self.annotation_metadata[instance_id] = {}
        self.annotation_metadata[instance_id]['category'] = category
        
        # Rebuild base visualization (without the editing instance)
        self.rebuild_visualizations()
        
        # Show editing mask on top
        self._update_editing_visualization()
        self.instance_selected.emit(instance_id, category)
    
    def _should_delete_empty_editing_instance(self):
        """Return whether an empty editing mask means the instance should be deleted."""
        if self.editing_instance_id <= 0:
            return False

        metadata = self.annotation_metadata.get(self.editing_instance_id, {})
        bbox = metadata.get('bbox')
        is_empty_placeholder = (
            metadata.get('bbox_only', False) and
            (bbox is None or bbox == [0, 0, 0, 0])
        )
        return self._editing_started_with_pixels or is_empty_placeholder

    def commit_editing(self):
        """Commit editing mask back to combined mask
        
        Merges the temporary editing mask back into the main combined mask.
        """
        # Flush any pending visualization updates first
        if self._viz_update_timer.isActive():
            self._viz_update_timer.stop()
            self._flush_pending_viz_update()
        
        if self.editing_instance_id <= 0 or self.editing_mask is None:
            return
        
        # Check if editing mask is empty (no pixels drawn)
        has_pixels = np.any(self.editing_mask > 0)
        
        if not has_pixels:
            instance_id = self.editing_instance_id
            category = self.editing_instance_category or self.annotation_metadata.get(
                instance_id, {}
            ).get('category', self.current_annotation_type)

            if self._should_delete_empty_editing_instance():
                self.delete_instance(instance_id, category=category)
                return

            # Empty editing mask from a real bbox-only instance - don't commit,
            # just exit editing mode and keep the bbox annotation.
            self.editing_mask = None
            self.editing_instance_id = -1
            self.editing_instance_category = None
            self._editing_started_with_pixels = False
            
            # Clear editing overlay cache
            if self._editing_overlay_cache is not None:
                del self._editing_overlay_cache
                self._editing_overlay_cache = None
            
            # Remove editing visualization
            if self.editing_mask_item:
                try:
                    if self.editing_mask_item.scene() == self.scene:
                        self.scene.removeItem(self.editing_mask_item)
                except (RuntimeError, AttributeError):
                    # Item already deleted by Qt or invalid
                    pass
                try:
                    self.editing_mask_item.setPixmap(None)
                except:
                    pass
                del self.editing_mask_item
                self.editing_mask_item = None
            
            # Rebuild visualization to restore bbox-only instances
            self.rebuild_visualizations()
            self._clear_brush_history()
            return
        
        # Create appropriate mask if it doesn't exist (e.g., when converting bbox-only to mask)
        if self.current_image is not None:
            h, w = self.current_image.shape[:2]
        else:
            return
        
        # Get category from metadata or use current annotation type
        category = self.editing_instance_category or self.annotation_metadata.get(
            self.editing_instance_id, {}
        ).get('category', self.current_annotation_type)
        
        # Get or create the appropriate mask array
        if category == 'chamber':
            if self.chamber_mask is None:
                self.chamber_mask = np.zeros((h, w), dtype=np.int32)
            mask_array = self.chamber_mask
        elif category == 'hive':
            if self.hive_mask is None:
                self.hive_mask = np.zeros((h, w), dtype=np.int32)
            mask_array = self.hive_mask
        elif category == 'pollen':
            if self.pollen_mask is None:
                self.pollen_mask = np.zeros((h, w), dtype=np.int32)
            mask_array = self.pollen_mask
        else:  # bee
            if self.bee_mask is None:
                self.bee_mask = np.zeros((h, w), dtype=np.int32)
            mask_array = self.bee_mask
        
        # Merge editing mask back into appropriate mask array
        editing_pixels = self.editing_mask > 0
        mask_array[editing_pixels] = self.editing_instance_id
        
        # Mark combined mask cache as dirty
        self._combined_mask_dirty = True
        
        # Invalidate cached instance IDs (in case this was a new instance)
        self._cached_instance_ids = None
        
        # Persist the resolved category in annotation_metadata so subsequent
        # get_annotations() calls always return the right category.
        if self.editing_instance_id not in self.annotation_metadata:
            self.annotation_metadata[self.editing_instance_id] = {}
        self.annotation_metadata[self.editing_instance_id]['category'] = category
        
        # If this was a bbox-only annotation, remove the bbox_only flag from metadata
        if self.editing_instance_id in self.annotation_metadata:
            if 'bbox_only' in self.annotation_metadata[self.editing_instance_id]:
                del self.annotation_metadata[self.editing_instance_id]['bbox_only']
        
        # Clear editing state and free memory
        old_editing_mask = self.editing_mask
        self.editing_mask = None
        self.editing_instance_id = -1
        self.editing_instance_category = None
        self._editing_started_with_pixels = False
        del old_editing_mask
        
        # Clear editing overlay cache
        if self._editing_overlay_cache is not None:
            del self._editing_overlay_cache
            self._editing_overlay_cache = None
        
        # Remove editing visualization
        if self.editing_mask_item:
            try:
                if self.editing_mask_item.scene() == self.scene:
                    self.scene.removeItem(self.editing_mask_item)
            except (RuntimeError, AttributeError):
                # Item already deleted by Qt or invalid
                pass
            try:
                self.editing_mask_item.setPixmap(None)
            except:
                pass
            del self.editing_mask_item
            self.editing_mask_item = None
        
        # Rebuild full visualization
        self.rebuild_visualizations()
        self._clear_brush_history()
        
        self.annotation_changed.emit()
    
    def _remove_editing_items(self):
        """Remove transient editing overlay items from the scene."""
        for attr_name in ('editing_mask_item', 'editing_bbox_item'):
            item = getattr(self, attr_name, None)
            if item:
                try:
                    if item.scene() == self.scene:
                        self.scene.removeItem(item)
                except (RuntimeError, AttributeError):
                    pass
                try:
                    item.setPixmap(None)
                except Exception:
                    pass
                setattr(self, attr_name, None)

        if self._editing_overlay_cache is not None:
            del self._editing_overlay_cache
            self._editing_overlay_cache = None

    def delete_instance(self, instance_id, category=None, rebuild=True, emit_changed=True):
        """Delete one instance from masks, metadata, bboxes, and edit state."""
        if instance_id is None or instance_id <= 0:
            return False

        instance_id = int(instance_id)
        changed = False

        if category is None:
            category = self.annotation_metadata.get(instance_id, {}).get(
                'category',
                self.selected_instance_category or self.current_annotation_type
            )

        if self.editing_instance_id == instance_id:
            self.editing_mask = None
            self.editing_instance_id = -1
            self.editing_instance_category = None
            self._editing_started_with_pixels = False
            self._clear_brush_history()
            self._remove_editing_items()
            changed = True

        for _mask_category, mask_attr in (
            ('bee', 'bee_mask'),
            ('chamber', 'chamber_mask'),
            ('hive', 'hive_mask'),
            ('pollen', 'pollen_mask'),
        ):
            mask_array = getattr(self, mask_attr, None)
            if mask_array is not None and np.any(mask_array == instance_id):
                mask_array[mask_array == instance_id] = 0
                changed = True

        if instance_id in self.annotation_metadata:
            del self.annotation_metadata[instance_id]
            changed = True

        if instance_id in self.mask_colors:
            del self.mask_colors[instance_id]
            changed = True

        for key in list(self.hidden_instance_keys):
            if (
                key == instance_id or
                (isinstance(key, tuple) and len(key) > 1 and key[1] == instance_id)
            ):
                self.hidden_instance_keys.discard(key)
        self.hidden_bbox_instance_ids.discard(instance_id)

        if instance_id in self.bbox_items_map:
            rect_item = self.bbox_items_map.pop(instance_id)
            try:
                if rect_item.scene() == self.scene:
                    self.scene.removeItem(rect_item)
            except (RuntimeError, AttributeError):
                pass
            changed = True

        if self.selected_bbox_id == instance_id:
            self.selected_bbox_id = None
        if self.selected_mask_idx == instance_id:
            self.selected_mask_idx = -1
            self.selected_instance_category = None
        if self.active_sam2_mask_idx == instance_id:
            self.active_sam2_mask_idx = -1

        if self.selection_border_item:
            try:
                if self.selection_border_item.scene() == self.scene:
                    self.scene.removeItem(self.selection_border_item)
            except (RuntimeError, AttributeError):
                pass
            self.selection_border_item = None

        for item in self.prompt_items:
            try:
                if item.scene() == self.scene:
                    self.scene.removeItem(item)
            except (RuntimeError, AttributeError):
                pass
        self.positive_points = []
        self.negative_points = []
        self.prompt_items = []

        self._combined_mask_dirty = True
        self._cached_instance_ids = None
        self._cached_bboxes = {
            key: bbox
            for key, bbox in self._cached_bboxes.items()
            if not (
                key == instance_id or
                (isinstance(key, tuple) and len(key) > 1 and key[1] == instance_id)
            )
        }

        if rebuild:
            self.rebuild_visualizations()
        if emit_changed and changed:
            self.annotation_changed.emit()

        return changed

    def _update_editing_visualization(self):
        """Update visualization of the editing mask
        
        Shows the editing mask as an overlay on top of other instances.
        Rebuilds the entire overlay - use for initialization or major changes.
        """
        if self.editing_mask is None or self.editing_instance_id <= 0:
            return
        
        # Remove previous editing visualization
        if self.editing_mask_item:
            try:
                if self.editing_mask_item.scene() == self.scene:
                    self.scene.removeItem(self.editing_mask_item)
            except (RuntimeError, AttributeError):
                # Item already deleted by Qt or invalid
                pass
            self.editing_mask_item = None
        
        # Create or rebuild cached overlay for editing mask
        h, w = self.editing_mask.shape
        
        # Get color for this instance
        category = self.editing_instance_category or self.annotation_metadata.get(
            self.editing_instance_id, {}
        ).get('category', self.current_annotation_type)
        color = self.ensure_instance_color(self.editing_instance_id, category)
        
        # Create full overlay and cache it
        self._editing_overlay_cache = np.zeros((h, w, 4), dtype=np.uint8)
        mask_pixels = self.editing_mask > 0
        alpha = min(self.mask_opacity, 255)
        self._editing_overlay_cache[mask_pixels] = [color[0], color[1], color[2], alpha]
        
        # Convert to QPixmap (use QImage.copy() to prevent memory leak)
        q_image = QImage(self._editing_overlay_cache.data, w, h, w * 4, QImage.Format.Format_RGBA8888)
        pixmap = QPixmap.fromImage(q_image.copy())
        
        # Add to scene on top of everything
        self.editing_mask_item = QGraphicsPixmapItem(pixmap)
        self.editing_mask_item.setZValue(125)  # Above regular masks and non-active boxes
        self.editing_mask_item.setVisible(self._is_editing_mask_visible())
        self.scene.addItem(self.editing_mask_item)
        self._draw_or_update_editing_bbox()
        
        # Update selection border for editing mask (skip if actively drawing)
        if not self._is_actively_drawing and self.selected_mask_idx == self.editing_instance_id:
            self.add_selection_border(self.editing_mask)
            self.update_instance_labels()

    def _is_editing_mask_visible(self):
        """Return whether the active editing overlay should be visible."""
        if self.editing_instance_id <= 0:
            return False
        category = self.editing_instance_category or self.annotation_metadata.get(
            self.editing_instance_id, {}
        ).get('category', self.current_annotation_type)
        return (
            self._segmentation_overlays_visible() and
            self.is_annotation_instance_visible(self.editing_instance_id, category)
        )

    def _sync_editing_mask_visibility(self):
        """Apply current global/class/instance visibility to the editing overlay."""
        if self.editing_mask_item:
            self.editing_mask_item.setZValue(125)
            self.editing_mask_item.setVisible(self._is_editing_mask_visible())
        self._sync_editing_bbox_visibility()
    
    def _flush_pending_viz_update(self):
        """Flush pending visualization update (called by timer)"""
        if self._dirty_pixels is not None:
            self._update_editing_visualization_incremental(self._dirty_pixels)
            self._dirty_pixels = None
    
    def _update_editing_visualization_incremental(self, affected_pixels):
        """Update only the affected region of the editing visualization
        
        Much faster than full rebuild - updates only changed pixels.
        
        Args:
            affected_pixels: Boolean mask of pixels that changed
        """
        if self.editing_mask is None or self.editing_instance_id <= 0:
            return
        
        # Initialize cache if needed
        if self._editing_overlay_cache is None:
            self._update_editing_visualization()
            return
        
        # Get color for this instance
        color = self.mask_colors.get(self.editing_instance_id, (255, 255, 255))
        alpha = min(self.mask_opacity, 255)
        
        # Update only affected pixels in the cached overlay
        # For brush: set pixels to color where editing_mask is non-zero
        # For eraser: set pixels to transparent where editing_mask is zero
        brush_pixels = affected_pixels & (self.editing_mask > 0)
        eraser_pixels = affected_pixels & (self.editing_mask == 0)
        
        self._editing_overlay_cache[brush_pixels] = [color[0], color[1], color[2], alpha]
        self._editing_overlay_cache[eraser_pixels] = [0, 0, 0, 0]
        
        # Update the pixmap from cached overlay (optimized: reuse buffer)
        h, w = self._editing_overlay_cache.shape[:2]
        # Create QImage directly from buffer without copy for better performance
        # The buffer is kept alive by self._editing_overlay_cache
        q_image = QImage(self._editing_overlay_cache.data, w, h, w * 4, QImage.Format.Format_RGBA8888)
        # Only copy when creating pixmap to avoid tearing
        pixmap = QPixmap.fromImage(q_image)
        
        # Update existing item
        if self.editing_mask_item:
            self.editing_mask_item.setPixmap(pixmap)
            self.editing_mask_item.setZValue(125)
        else:
            # Create item if it doesn't exist
            self.editing_mask_item = QGraphicsPixmapItem(pixmap)
            self.editing_mask_item.setZValue(125)
            self.editing_mask_item.setVisible(self._is_editing_mask_visible())
            self.scene.addItem(self.editing_mask_item)
        self._draw_or_update_editing_bbox()
        self._sync_editing_mask_visibility()
    
    def clear_selected_instance(self):
        """Remove the selected instance from the combined mask"""
        if self.selected_mask_idx <= 0:
            return
        self.delete_instance(
            self.selected_mask_idx,
            category=self.selected_instance_category
        )
        
    def undo(self):
        """Undo the last brush/eraser/fill edit for the active instance."""
        if self._viz_update_timer.isActive():
            self._viz_update_timer.stop()
            self._flush_pending_viz_update()

        if self.editing_mask is None or self.editing_instance_id <= 0:
            print("Undo skipped: no active brush-edit instance.")
            return
        if not self.brush_undo_stack:
            print("Undo skipped: no brush history available.")
            return

        current_snapshot = self._make_editing_mask_snapshot()
        previous_snapshot = self.brush_undo_stack.pop()
        if current_snapshot is not None:
            self.brush_redo_stack.append(current_snapshot)
            self._trim_brush_stack(self.brush_redo_stack)

        self._brush_step_active = False
        self._brush_step_start_snapshot = None
        self._restore_editing_mask_snapshot(previous_snapshot)
        
    def redo(self):
        """Redo the last undone brush/eraser/fill edit for the active instance."""
        if self._viz_update_timer.isActive():
            self._viz_update_timer.stop()
            self._flush_pending_viz_update()

        if self.editing_mask is None or self.editing_instance_id <= 0:
            print("Redo skipped: no active brush-edit instance.")
            return
        if not self.brush_redo_stack:
            print("Redo skipped: no brush redo history available.")
            return

        current_snapshot = self._make_editing_mask_snapshot()
        next_snapshot = self.brush_redo_stack.pop()
        if current_snapshot is not None:
            self.brush_undo_stack.append(current_snapshot)
            self._trim_brush_stack(self.brush_undo_stack)

        self._brush_step_active = False
        self._brush_step_start_snapshot = None
        self._restore_editing_mask_snapshot(next_snapshot)
        
    def set_brush_size(self, size):
        """Set brush size"""
        self.brush_size = max(1, int(size))
        self._update_brush_cursor_preview()
    
    def set_mask_opacity(self, opacity):
        """Set mask opacity and refresh all visualizations"""
        self.mask_opacity = opacity
        # Update all visualizations with new opacity
        self.rebuild_visualizations()
        
    def add_prompt_marker(self, x, y, is_positive):
        """Add a visual marker for SAM2 prompt point"""
        from PyQt6.QtWidgets import QGraphicsEllipseItem
        
        # Create circular marker
        radius = 5
        color = QColor(0, 255, 0) if is_positive else QColor(255, 0, 0)
        
        marker = QGraphicsEllipseItem(x - radius, y - radius, radius * 2, radius * 2)
        marker.setPen(QPen(color, 2))
        marker.setBrush(QBrush(color))
        marker.setZValue(10)  # Above everything
        
        self.scene.addItem(marker)
        self.prompt_items.append(marker)
        
    def clear_prompt_points(self):
        """Clear all SAM2 prompt points and remove active mask (only if it's a new temporary mask)"""
        # Remove visual markers
        for item in self.prompt_items:
            try:
                if item.scene() == self.scene:
                    self.scene.removeItem(item)
            except (RuntimeError, AttributeError):
                # Item already deleted by Qt or invalid
                pass
        
        # Clear storage
        self.positive_points = []
        self.negative_points = []
        self.prompt_items = []
        
        # Remove the active SAM2 mask ONLY if it's not a selected instance
        # (i.e., it's a temporary mask being built, not an existing one being edited)
        if self.active_sam2_mask_idx > 0 and self.combined_mask is not None:
            # Only delete if it's NOT the selected instance (selected instances should be preserved)
            if self.active_sam2_mask_idx != self.selected_mask_idx:
                # Remove this instance from combined mask
                self.combined_mask[self.combined_mask == self.active_sam2_mask_idx] = 0
                
                # Remove color entry
                if self.active_sam2_mask_idx in self.mask_colors:
                    del self.mask_colors[self.active_sam2_mask_idx]
                
                # Rebuild visualizations
                self.rebuild_visualizations()
        
        # Remove selection border
        if self.selection_border_item:
            try:
                if self.selection_border_item.scene() == self.scene:
                    self.scene.removeItem(self.selection_border_item)
            except (RuntimeError, AttributeError):
                # Item already deleted by Qt or invalid
                pass
            self.selection_border_item = None
        
        # Reset active mask index
        self.active_sam2_mask_idx = -1
        self.selected_instance_category = None
        self._clear_brush_history()
    
    def start_new_sam2_instance(self):
        """Start annotating a new SAM2 instance"""
        # Clear prompts and reset for new instance
        for item in self.prompt_items:
            try:
                if item.scene() == self.scene:
                    self.scene.removeItem(item)
            except (RuntimeError, AttributeError):
                # Item already deleted by Qt or invalid
                pass
        
        self.positive_points = []
        self.negative_points = []
        self.prompt_items = []
        self.active_sam2_mask_idx = -1
        self.selected_mask_idx = -1
        self.selected_instance_category = None
        self._clear_brush_history()
        
        # Remove selection border
        if self.selection_border_item:
            try:
                if self.selection_border_item.scene() == self.scene:
                    self.scene.removeItem(self.selection_border_item)
            except (RuntimeError, AttributeError):
                # Item already deleted by Qt or invalid
                pass
            self.selection_border_item = None
    
    def get_prompt_points(self):
        """Get all accumulated prompt points for SAM2"""
        return {
            'positive': self.positive_points.copy(),
            'negative': self.negative_points.copy()
        }
    
    def set_show_segmentations(self, show):
        """Enable or disable segmentation mask display"""
        self.set_annotation_overlay_visibility(show_segmentations=show)

    def set_show_bboxes(self, show):
        """Enable or disable bounding box display"""
        self.set_annotation_overlay_visibility(show_bboxes=show)

    def set_annotation_overlay_visibility(self, show_segmentations=None, show_bboxes=None):
        """Set segmentation and bbox layer visibility together.

        Class-level and per-instance visibility are intentionally not changed here.
        """
        if show_segmentations is None:
            show_segmentations = self.show_segmentations
        if show_bboxes is None:
            show_bboxes = self.show_bboxes

        show_segmentations = bool(show_segmentations)
        show_bboxes = bool(show_bboxes)
        segmentation_changed = (
            self.show_segmentations != show_segmentations or
            self.masks_visible != show_segmentations
        )
        bboxes_changed = self.show_bboxes != show_bboxes

        if self._viz_update_timer.isActive():
            self._viz_update_timer.stop()
            self._flush_pending_viz_update()

        if not show_segmentations and self.show_segmentations:
            self.labels_visible_before_hiding = self.labels_visible

        self.show_segmentations = show_segmentations
        # Keep the old quick-mask state as an alias for the real segmentation
        # switch so Space, the toolbar checkbox, and the menu cannot disagree.
        self.masks_visible = show_segmentations
        self.show_bboxes = show_bboxes

        if not show_bboxes:
            self._deselect_bbox()

        self.rebuild_visualizations()

        if segmentation_changed:
            self.masks_visibility_changed.emit(show_segmentations)
        if bboxes_changed:
            self.bboxes_visibility_changed.emit(show_bboxes)
    
    def set_bbox_mode(self, enabled):
        """DEPRECATED: Use set_show_bboxes instead"""
        self.set_show_bboxes(enabled)
    
    def _get_bbox_at_pos(self, pos):
        """Get bbox ID at scene position.

        If multiple bboxes contain the point, keep the active selection when it
        is one of the hits. Otherwise the click is ambiguous and returns None.
        
        Args:
            pos: QPointF scene position
            
        Returns:
            mask_id of bbox at position, or None
        """
        # Clean up any deleted items while iterating
        to_remove = []
        hits = []
        
        for mask_id, rect_item in self.bbox_items_map.items():
            try:
                if rect_item.rect().contains(pos):
                    hits.append(mask_id)
            except (RuntimeError, AttributeError):
                # Item has been deleted by Qt or is invalid
                to_remove.append(mask_id)
        
        # Clean up deleted items from map
        for mask_id in to_remove:
            del self.bbox_items_map[mask_id]

        if not hits:
            return None

        for active_id in (
            self.editing_instance_id,
            self.selected_bbox_id,
            self.selected_mask_idx,
        ):
            if active_id and active_id in hits:
                return active_id

        if len(hits) == 1:
            return hits[0]
        
        return None
    
    def _select_bbox(self, mask_id, zoom=True):
        """Select a bbox for editing
        
        Args:
            mask_id: ID of bbox to select
            zoom: Whether to zoom to the bbox after selecting it.
        """
        if mask_id not in self.bbox_items_map:
            return
        
        # Deselect previous bbox
        if self.selected_bbox_id and self.selected_bbox_id != mask_id:
            self._deselect_bbox()
        
        self.selected_bbox_id = mask_id
        self.selected_mask_idx = mask_id  # Keep instance selection in sync
        self.selected_instance_category = self.annotation_metadata.get(mask_id, {}).get(
            'category', self.selected_instance_category or self.current_annotation_type
        )
        rect_item = self.bbox_items_map[mask_id]
        
        # Highlight selected bbox by increasing pen width
        color = self.mask_colors.get(mask_id, (255, 255, 0))
        pen = QPen(QColor(color[0], color[1], color[2]))
        pen.setWidth(6)  # Thicker for selected bbox
        rect_item.setPen(pen)
        rect_item.setZValue(101)  # Bring to front
        
        # Always allow bbox editing — the bbox is independent metadata
        # even when the instance also has a segmentation mask
        self._create_bbox_handles()
        self.setToolTip("Drag corners to resize, drag center to move bbox")
        
        if zoom:
            rect = rect_item.rect()
            self.zoom_to_rect(int(rect.x()), int(rect.y()),
                              int(rect.width()), int(rect.height()), padding=100)
        self.instance_selected.emit(mask_id, self.selected_instance_category)
    
    def _deselect_bbox(self):
        """Deselect currently selected bbox"""
        if self.selected_bbox_id and self.selected_bbox_id in self.bbox_items_map:
            rect_item = self.bbox_items_map[self.selected_bbox_id]
            
            # Reset pen to normal
            color = self.mask_colors.get(self.selected_bbox_id, (255, 255, 0))
            pen = QPen(QColor(color[0], color[1], color[2]))
            pen.setWidth(4)  # Normal width
            rect_item.setPen(pen)
            rect_item.setZValue(100)  # Normal z-order
        
        # Remove handles
        for handle in self.bbox_handles:
            try:
                self.scene.removeItem(handle)
            except RuntimeError:
                pass
        self.bbox_handles = []
        
        self.selected_bbox_id = None
        if self.show_bboxes:
            self.setToolTip("Click on a bbox to select and edit")
    
    def _create_new_bbox(self, rect):
        """Create a new bbox annotation from a drawn rectangle
        
        Args:
            rect: QRectF of the drawn bbox
        """
        from PyQt6.QtGui import QPen, QColor
        
        # New bbox drawing only attaches to an explicitly selected instance.
        if self.selected_mask_idx <= 0:
            print("BBox creation skipped: create or select an instance first.")
            return

        mask_id = self.selected_mask_idx
        
        # Update or create metadata for this instance
        if mask_id not in self.annotation_metadata:
            self.annotation_metadata[mask_id] = {}

        # Preserve existing category if set; otherwise use current annotation type
        existing_category = self.annotation_metadata[mask_id].get('category', self.current_annotation_type)
        color = self.ensure_instance_color(mask_id, existing_category)

        # Create bbox metadata
        bbox = [int(rect.x()), int(rect.y()), int(rect.width()), int(rect.height())]

        self.annotation_metadata[mask_id].update({
            'mask_id': mask_id,
            'bbox': bbox,
            'bbox_only': True,  # Mark as bbox-only until segmentation is added
            'category': existing_category
        })
        
        # Invalidate cached instance IDs since we created a new one
        self._cached_instance_ids = None
        
        # Create rectangle item for visualization
        rect_item = OutwardBBoxRectItem(rect)
        pen = QPen(QColor(color[0], color[1], color[2]))
        pen.setWidth(4)
        rect_item.setPen(pen)
        rect_item.setZValue(100)
        
        self.scene.addItem(rect_item)
        self.mask_items.append(rect_item)
        self.bbox_items_map[mask_id] = rect_item
        
        # Update labels if visible
        self.update_instance_labels()
        
        # Select the newly created bbox
        self._select_bbox(mask_id)
        
        # Emit annotation changed signal
        self.annotation_changed.emit()
        
        print(f"Created new bbox with ID {mask_id}: {bbox}")
    
    def _create_bbox_handles(self):
        """Create corner handles for selected bbox"""
        from PyQt6.QtWidgets import QGraphicsEllipseItem
        
        # Remove old handles
        for handle in self.bbox_handles:
            try:
                self.scene.removeItem(handle)
            except RuntimeError:
                pass
        self.bbox_handles = []
        
        if not self.selected_bbox_id or self.selected_bbox_id not in self.bbox_items_map:
            return
        
        rect = self.bbox_items_map[self.selected_bbox_id].rect()
        handle_size = 10
        handle_color = QColor(255, 255, 0)  # Yellow handles
        
        # Create corner handles
        corners = {
            'tl': rect.topLeft(),
            'tr': rect.topRight(),
            'bl': rect.bottomLeft(),
            'br': rect.bottomRight()
        }
        
        for corner_id, corner_pos in corners.items():
            handle = QGraphicsEllipseItem(
                corner_pos.x() - handle_size/2,
                corner_pos.y() - handle_size/2,
                handle_size, handle_size
            )
            handle.setBrush(QBrush(handle_color))
            handle.setPen(QPen(QColor(0, 0, 0), 1))
            handle.setZValue(102)  # Above bbox
            handle.setData(0, corner_id)  # Store corner ID
            self.scene.addItem(handle)
            self.bbox_handles.append(handle)
    
    def _update_bbox_handles(self):
        """Update corner handle positions"""
        if not self.selected_bbox_id or self.selected_bbox_id not in self.bbox_items_map:
            return
        
        rect = self.bbox_items_map[self.selected_bbox_id].rect()
        handle_size = 10
        
        corners = {
            'tl': rect.topLeft(),
            'tr': rect.topRight(),
            'bl': rect.bottomLeft(),
            'br': rect.bottomRight()
        }
        
        for handle in self.bbox_handles:
            corner_id = handle.data(0)
            if corner_id in corners:
                corner_pos = corners[corner_id]
                handle.setRect(
                    corner_pos.x() - handle_size/2,
                    corner_pos.y() - handle_size/2,
                    handle_size, handle_size
                )
    
    def _get_bbox_handle_at_pos(self, pos):
        """Get bbox handle at scene position
        
        Args:
            pos: QPointF scene position
            
        Returns:
            Handle ID ('tl', 'tr', 'bl', 'br') or None
        """
        for handle in self.bbox_handles:
            try:
                if handle.contains(pos):
                    return handle.data(0)
            except RuntimeError:
                # Handle has been deleted by Qt
                continue
        return None
    
    
    def set_annotation_type_visibility(self, annotation_type, visible, rebuild=True):
        """Set visibility for a specific annotation type
        
        Args:
            annotation_type: Annotation type ('bee', 'hive', or 'chamber')
            visible: Boolean visibility state
            rebuild: If True, rebuild visualizations immediately (default: True)
        """
        if annotation_type in self.annotation_type_visibility:
            self.annotation_type_visibility[annotation_type] = visible
            if rebuild:
                self.rebuild_visualizations()
    
    
    # Chamber and Hive mask management methods
    
    def set_annotation_type(self, annotation_type, rebuild=True):
        """Set the current annotation type (bee/chamber/hive)
        
        Args:
            annotation_type: 'bee', 'chamber', or 'hive'
            rebuild: If True, rebuild visualizations immediately (default: True)
        """
        if annotation_type not in ['bee', 'chamber', 'hive', 'pollen']:
            return

        if annotation_type == self.current_annotation_type:
            return  # No change — nothing to do

        # Commit any active editing before switching type so the
        # edited mask is written back to the correct array and the
        # editing overlay is cleared.
        if self.editing_instance_id > 0:
            self.commit_editing()

        # Deselect current instance — it belongs to the old type
        self.selected_mask_idx = -1
        self.selected_instance_category = None
        self.active_sam2_mask_idx = -1

        self.current_annotation_type = annotation_type

        # Rebuild so only the new type's overlay is shown
        if rebuild:
            self.rebuild_visualizations()
    
    def get_annotation_type(self):
        """Get the current annotation type
        
        Returns:
            String: 'bee', 'chamber', or 'hive'
        """
        return self.current_annotation_type
