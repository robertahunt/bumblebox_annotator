...
class YOLOInstanceFocusedToolbar(QWidget):
    """Toolbar for YOLO instance-focused mask refinement"""
    ...
    def init_ui(self):
        ...
        self.load_model_btn.setToolTip("Load a trained instance-focused YOLO segmentation model checkpoint (.pt file)")
        ...
        self.train_menu_btn.setToolTip("Train an instance-focused refinement model")
        
        # Create menu
        train_menu = QMenu(self)
        
        # Add train action
        train_action = QAction("Train Instance-Focused Model", self)
        train_action.setToolTip("Train a new instance-focused YOLO model on individual bee crops")
        ...
        self.padding_spinbox.setToolTip("Padding around instance bounding box when cropping (in pixels)")
        ...
        # Refine mask button
        self.refine_btn = QPushButton("Refine Instance")
        self.refine_btn.setToolTip(
            "Refine the currently selected instance mask using the instance-focused model.\n"
            "A crop around the instance will be created, resized, and processed.\n"
            "Only the primary instance will be segmented (other bees in crop are ignored)."
        )
        ...
        # Refine all button
        self.refine_all_btn = QPushButton("Refine All Bee Instances")
        self.refine_all_btn.setToolTip(
            "Refine all bee instances in the current frame using per-pixel probability.\n"
            "Each bee instance is predicted independently, then overlaps are resolved\n"
            "by choosing the prediction with highest confidence for each pixel.\n"
            "Skips hive and chamber annotations."
        )
    ...
    def on_train_model(self):
        ...
        while parent_window and not hasattr(parent_window, 'train_yolo_model_instance_focused'):
            parent_window = parent_window.parent()
        
        if parent_window and hasattr(parent_window, 'train_yolo_model_instance_focused'):
            parent_window.train_yolo_model_instance_focused()
        else:
            QMessageBox.warning(
                self,
    ...
    def load_checkpoint(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            self,
            "Select Instance-Focused YOLO Checkpoint",
            str(Path.home()),
        ...
        try:
            ...
            if show_dialogs:
                QMessageBox.information(
                    self,
                    "Model Loaded",
                    f"Successfully loaded instance-focused YOLO model:\n{model_name}\n\n"
                    f"Task: {model.task}\n"
                    f"Ready for instance-focused refinement!"
                )
            else:
                print(f"Instance-focused YOLO loaded successfully: {model_name} (Task: {model.task})")
